...
public int valid() {
  int y = 10;
  int x; // x  объявлен здесь
  x = 3; // x инициализируется здесь
  int z; // z объявлен здесь, но никогда не инициализируется и не используется
  int reply = x + y;
  return reply;
}
```
...
public void findAnswer(boolean check) {
   ...
   System.out.println(onlyOneBranch); // НЕ КОМПИЛИРУЕТСЯ
}
```

Переменная answer инициализируется в обеих ветвях оператора if, так что компилятор вполне доволен. Он знает, что 
независимо от того, является ли проверка истинной или ложной, значение answer будет установлено в какое-либо значение 
...
об операторе if в главе 3 «Принятие решений».
...
### Передача параметров конструктора и метода
...
```
public void findAnswer(boolean check) {}
    Взгляните на следующий метод checkAnswer() в том же классе:
public void checkAnswer() {
  boolean value;
  findAnswer(value); // НЕ КОМПИЛИРУЕТСЯ
}
```
...
### Определение переменных экземпляра и класса
...
### Определение типа с помощью var
...
public class VarKeyword {
    var tricky = "Hello"; // НЕ КОМПИЛИРУЕТСЯ
}
```
...
8:    var number = 7;
9:    number = 4;
10:   number = "five"; // НЕ КОМПИЛИРУЕТСЯ
11: }
```
...
Этот пример корректен и компилируется, но мы считаем, что объявление и инициализация silly происходят в одной строке.
...
5:    int a, var b = 3;   // НЕ КОМПИЛИРУЕТСЯ
6:    var n = null;       // НЕ КОМПИЛИРУЕТСЯ
7: }
```
...
```
public int addition(var a, var b) {   // НЕ КОМПИЛИРУЕТСЯ
   return a + b;
}
```

В этом примере a и b являются параметрами метода. Это не локальные переменные. Обращайте внимание на переменную var, 
используемую с конструкторами, параметрами метода или переменными экземпляра. Использование var в одном из этих 
...
6:     }   // bitesOfCheese здесь выходит за рамки
7:     System.out.println(bitesOfCheese);   // НЕ КОМПИЛИРУЕТСЯ
8: }
```
...
22:        }
23:     }
24:     System.out.println(teenyBit); // НЕ КОМПИЛИРУЕТСЯ
25: }
```
...
На экзамене будет предпринята попытка обмануть вас различными вопросами, касающимися объема знаний. Вероятно, вы увидите 
вопрос, который, по-видимому, касается чего-то сложного и не может быть скомпилирован, потому что одна из переменных 
выходит за рамки области видимости.
...
#### Таблица 1.10 - Отслеживание объёма по блокам
...
### Применение области видимости к классам
...
В этом классе у нас есть одна переменная класса, MAX_LENGTH; одна переменная экземпляра, length; и две локальные 
...
Понял все это? Давайте рассмотрим правила, касающиеся области видимости:
...
Есть бумага? Хорошо, давайте начнем. В строке 3 напишите один и два (просто слова — не нужно ни прямоугольников, 
...
В строке 6 переменная one изменяется и указывает на «b». Либо сотрите, либо зачеркните стрелку от **one** и нарисуйте 
новую стрелку от **one** до «b». В строке 7 у нас есть новая переменная, поэтому напишите слово «три» и 
...
Умейте писать код, используя метод main(). Метод main() обычно записывается как public static void main(String[] args).
...
строке, где она объявлена, и, хотя она может изменить значение, она не может изменить тип. Переменная не может быть инициализирована null значением без типа, а также не может использоваться в объявлениях нескольких переменных.
