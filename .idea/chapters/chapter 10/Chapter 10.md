...
#### Таблица 10.9 - Методы Spliterator

|                 Метод                  |                                                                                                                    Описание                                                                                                                    |  
|:--------------------------------------:|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------:|
|       Spliterator<T> trySplit()        | Возвращает Spliterator, содержащий в идеале половину данных, которые удаляются из текущего Spliterator. <br/> Этот метод может быть вызван несколько раз и в конечном итоге вернет значение null, когда данные больше не поддаются разделению. |
| void forEachRemaining( Consumer<T> c)  |                                                                                                Обрабатывает оставшиеся элементы в Spliterator.                                                                                                 |
|   boolean tryAdvance(Consumer<T> c)    |                                                                     Обрабатывает один элемент из Spliterator, если таковой остался. Возвращает, был ли обработан элемент.                                                                      |

Теперь давайте посмотрим на пример, где мы делим сумку на три:
...
В строках 12 и 13 мы определяем List. Строки 14 и 15 создают две ссылки Spliterator. Первый — это оригинальный мешочек, 
в котором собраны все семь элементов. Второй — это разделение оригинальной сумки: примерно половина элементов спереди 
помещается в сумку Эммы. Затем мы печатаем три содержимого сумки Эммы в строке 16.

Наш первоначальный пакет с едой теперь содержит четыре элемента. Мы создаем новый Spliterator в строке 18 и помещаем 
первые два элемента в сумку Джилл. Мы используем tryAdvance() в строке 19 для вывода одного элемента, а затем строка 20 
печатает все оставшиеся элементы (остался только один!).

Мы начали с семи элементов, удалили три, а затем еще два. В результате у нас остается два элемента в исходной сумке, 
созданной в строке 14. Эти два элемента выводятся в строке 22.

Теперь давайте попробуем пример со Stream. Это сложный способ распечатать 123:
...
Вы могли заметить, что это бесконечный поток. Без проблем! Spliterator распознает, что поток бесконечен, и не пытается 
дать вам половину. Вместо этого newBag содержит большое количество элементов. Мы получаем первые три, поскольку трижды 
вызываем tryAdvance(). Было бы плохой идеей вызывать forEachRemaining() для бесконечного потока!

Обратите внимание, что Spliterator может иметь ряд характеристик, таких как CONCURRENT, ORDERED, SIZED и SORTED. На 
экзамене вы увидите только простой Spliterator. Например, наш бесконечный поток не был SIZED.

### Сбор результатов

Вы почти закончили изучение потоков. Последняя тема основана на том, что вы уже узнали, для группировки результатов. 
В начале главы вы видели терминальную операцию collect(). Существует множество предопределенных сборщиков, в том числе 
показанные в таблице 10.10. Эти сборщики доступны через статические методы класса Collectors. В следующих разделах мы 
рассмотрим различные типы коллекторов. Для простоты мы исключили общие типы.

_Есть еще один сборщик, который называется reducing(). Для сдачи экзамена это знать не обязательно. Это общая скидка на 
случай, если все предыдущие сборщики не удовлетворят вашим потребностям._

#### Использование базовых коллекционеров

К счастью, многие из этих коллекционеров работают таким же образом. Давайте рассмотрим пример:
...
Обратите внимание, что предопределенные сборщики находятся в классе Collectors, а не в интерфейсе Collector. Это 
распространенная тема, которую вы видели в разделе Collection против Collections. Фактически, вы снова увидите этот 
шаблон в главе 14 при работе с Paths, Path и другими связанными типами.

#### Таблица 10.10 - Методы Spliterator

|                                                                         Сборщик                                                                         |                                                                  Описание                                                                   |                   Возвращаемое значение при передаче collect                   |  
|:-------------------------------------------------------------------------------------------------------------------------------------------------------:|:-------------------------------------------------------------------------------------------------------------------------------------------:|:------------------------------------------------------------------------------:|
|                    averagingDouble( ToDoubleFunction f) <br/> averagingInt (ToIntFunction f) <br/> averagingLong (ToLongFunction f)                     |                                        Вычисляет среднее значение для трех основных типов примитивов                                        |                                     Double                                     | 
|                                                                       counting()                                                                        |                                                      Подсчитывает количество элементов                                                      |                                      Long                                      |
|                                                           filtering(Predicate p, Collector c)                                                           |                                           Применяет фильтр перед вызовом нижестоящего коллектора                                            |                                       R                                        |
|                groupingBy(Function f) <br/> groupingBy(Function f, Collector dc) <br/>  groupingBy(Function f, Supplier s, Collector dc)                |    Создает группировку карт по указанной функции с дополнительным поставщиком типа карты и дополнительным сборщиком нисходящего потока.     |                                Map<K, List<T>>                                 |
|                                                                joining(CharSequence cs)                                                                 |                         Создает одну строку, используя cs в качестве разделителя между элементами, если он указан.                          |                                     String                                     |
|                                                     maxBy(Comparator c) <br/>  minBy(Comparator c)                                                      |                                                  Находит самые большие/наименьшие элементы                                                  |                                  Optional<T>                                   |
|                                                            mapping(Function f, Collector dc)                                                            |                                                  Добавляет еще один уровень коллекционеров                                                  |                                   Collector                                    |
|                                      partitioningBy(Predicate p) <br/>  partitioningBy(Predicate p, Collector dc)                                       |                     Создает группировку карт по указанному предикату с необязательным дальнейшим нисходящим коллектором                     |                             Map<Boolean, List<T>>                              |
|                 summarizingDouble( ToDoubleFunction f) <br/> summarizingInt( ToIntFunction f) <br/> summarizingLong( ToLongFunction f)                  |                                             Вычисляет среднее, минимальное, максимальное и т.д.                                             | DoubleSummaryStatistics <br/> IntSummaryStatistics <br/> LongSummaryStatistics |
|                        summingDouble( ToDoubleFunction f) <br/> summingInt(ToIntFunction f) <br/> summingLong(ToLongFunction f)                         |                                          Вычисляет сумму для наших трех основных типов примитивов                                           |                        Double <br/> Integer <br/> Long                         |
|                                                    teeing(Collector c1, Collector c2, BiFunction f)                                                     |                                       Работает с результатами двух сборщиков для создания нового типа                                       |                                       R                                        |
|                                                                 toList() <br/>  toSet()                                                                 |                                                 Создает произвольный тип списка или набора                                                  |                                List <br/>  Set                                 |
|                                                                toCollection(Supplier s)                                                                 |                                                      Создает коллекцию указанного типа                                                      |                                   Collection                                   |
| toMap(Function k, Function v) <br/> toMap(Function k, Function v, BinaryOperator m) <br/>  toMap(Function k, Function v, BinaryOperator m, Supplier s)  | Создает карту, используя функции для сопоставления ключей, значений, дополнительную функцию слияния и необязательный поставщик типа карты.  |                                      Map                                       |


Мы передаем предопределенный сборщик joining() методу collect(). Затем все элементы потока объединяются в строку с 
указанным разделителем между каждым элементом. Важно передать Collector методу collect. Он существует для того, чтобы 
помогать собирать элементы. Collector ничего не делает сам по себе.
...
Схема та же самая. Мы передаем сборщик в метод collect(), и он выполняет за нас среднее значение. На этот раз нам нужно 
было передать функцию, чтобы сообщить сборщику, что нужно усреднять. Мы использовали ссылку на метод, которая при 
выполнении возвращает целое число. В случае примитивных потоков результат усреднения всегда был double, независимо от 
того, какой тип усредняется. Для коллекционеров это Double, поскольку им нужен Object.

Часто вам приходится работать с кодом, написанным без потоков. Это означает, что он ожидает тип Collection, а не тип 
Stream. Без проблем. Вы по-прежнему можете выразить себя с помощью потока, а затем в конце преобразовать его в 
коллекцию.
...
На этот раз у нас есть все три части потокового конвейера. Stream.of() является источником потока. Промежуточная 
операция — filter(). Наконец, терминальная операция collect() создает TreeSet. Если бы нас не волновало, какую 
реализацию Set мы получим, мы могли бы вместо этого написать Collectors.toSet().

На этом этапе вы сможете использовать все коллекторы из таблицы 10.10, кроме groupingBy(), mapping(), partitioningBy(), 
toMap() и teeing().

### Сбор в карты

Код, использующий Collectors, включающий карты, может оказаться довольно длинным. Мы будем строить его медленно. Прежде 
чем переходить к следующему, убедитесь, что вы поняли каждый пример. Начнем с простого примера создания карты из потока:
...
При создании карты необходимо указать две функции. Первая функция сообщает сборщику, как создать ключ. В нашем примере 
мы используем предоставленную строку в качестве ключа. Вторая функция сообщает сборщику, как создать значение. В нашем 
примере мы используем длину строки в качестве значения.

_Возврат того же значения, переданного в лямбда-выражение, является обычной операцией, поэтому Java предоставляет для 
этого метод. Вы можете переписать s -> s как Function.identity(). Оно не короче и может быть яснее, а может и не быть, 
поэтому решайте сами, стоит ли его использовать._

Теперь мы хотим сделать обратное и сопоставить длину имени животного с самим именем. Наша первая неверная попытка 
показана здесь:
...
Запуск этого приводит к исключению, похожему на следующее:
...
В чем дело? Два названия животных имеют одинаковую длину. Мы не говорили Java, что делать. Должен ли коллекционер 
выбрать первый попавшийся? Последний, кого он встретит? Объединить два? Поскольку сборщик понятия не имеет, что делать,
он «решает» проблему, генерируя исключение и делая его нашей проблемой. Как заботливо. Предположим, что нам нужно 
создать строку с названиями животных, разделенную запятыми. Мы могли бы написать это:
...
Так получилось, что возвращенная карта является HashMap. Такое поведение не гарантируется. Предположим, мы хотим 
потребовать, чтобы код вместо этого возвращал TreeMap. Без проблем. Мы просто добавим ссылку на конструктор в качестве 
параметра:
...
На этот раз мы получаем тот тип, который указали. С нами до сих пор? Этот код длинный, но не особенно сложный. 
Мы обещали вам, что код будет длинным!

#### Группировка, разделение и сопоставление

Отличная работа, зайти так далеко. Создатели экзамена любят спрашивать о groupingBy() и partitioningBy(), поэтому 
убедитесь, что вы хорошо понимаете эти разделы. Теперь предположим, что мы хотим получить группы имен по их длине. Мы 
можем сделать это, сказав, что хотим сгруппировать по длине.
...
Коллектор groupingBy() сообщает методу collect(), что он должен сгруппировать все элементы потока в Map. Функция 
определяет ключи в Map. Каждое значение в Map представляет собой List всех записей, соответствующих этому ключу.

_Обратите внимание, что функция, которую вы вызываете в groupingBy(), не может возвращать значение null. Он 
не допускает null ключей._

Предположим, что нам не нужен List в качестве значения на карте, а вместо этого мы предпочитаем Set. Без проблем. Есть 
еще одна сигнатура метода, которая позволяет нам передавать нижестоящий сборщик. Это второй коллекционер, который делает 
с ценностями что-то особенное.
...
Мы даже можем изменить тип возвращаемой Map с помощью еще одного параметра.
...
Это очень гибко. Что, если мы захотим изменить тип возвращаемого объекта Map, но оставим тип значений в виде List? 
Специального метода для этого не существует, потому что его достаточно легко написать, используя существующие.
...
Разделение является частным случаем группировки. При секционировании возможны только две группы: истинные и ложные. 
Секционирование похоже на разделение списка на две части.

Предположим, мы делаем табличку, которую будем размещать возле выставки каждого животного. У нас есть знаки двух 
размеров. Можно разместить имена, состоящие из пяти или менее символов. Другой необходим для более длинных имен. Мы 
можем разбить список по тому, какой знак нам нужен.
...
Здесь мы передаем Predicate с логикой, к какой группе принадлежит каждое имя животного. Теперь предположим, что мы 
выяснили, как использовать другой шрифт, и теперь на меньшем знаке могут поместиться семь символов. Не волнуйтесь. Мы 
просто меняем Predicate.
...
Обратите внимание, что в карте по-прежнему есть два ключа — по одному для каждого логического значения. Бывает так, что 
одно из значений представляет собой пустой список, но оно все еще существует. Как и в случае с groupingBy(), мы можем 
изменить тип List на другой.
...
В отличие от groupingBy(), мы не можем изменить тип возвращаемого Map. Однако на карте всего два ключа, так имеет ли 
значение, какой тип Map мы используем?

Вместо использования нижестоящего сборщика для указания типа мы можем использовать любой из уже показанных сборщиков. 
Например, мы можем сгруппировать имена животных по длине, чтобы увидеть, сколько символов каждой длины у нас есть.
...
**Отладка сложных дженериков**

При работе с collect() часто используется множество уровней обобщений, что делает ошибки компилятора нечитаемыми. Вот 
три полезных метода, как справиться с этой ситуацией:

+ Начните с простого утверждения и продолжайте дополнять его. Внося одно крошечное изменение за раз, вы узнаете, какой 
код вызвал ошибку.
+ Выделите части утверждения в отдельные утверждения. Например, попробуйте написать 
Collectors.groupingBy(String::length, Collectors.counting());. Если он скомпилируется, вы знаете, что проблема в 
другом. Если он не скомпилируется, вам придется использовать гораздо более короткий оператор для устранения неполадок.
+ Используйте общие подстановочные знаки для типа возвращаемого значения последнего оператора: например, Map<?, ?>. 
Если только это изменение позволит скомпилировать код, вы поймете, что проблема заключается в том, что тип возвращаемого 
значения не соответствует вашим ожиданиям.


Наконец, есть сборщик mapping(), который позволяет нам спуститься на уровень ниже и добавить еще один сборщик. 
Предположим, мы хотели получить первую букву первого животного в алфавитном порядке каждой длины. Почему? Возможно, для 
случайной выборки. Примеры в этой части экзамена также довольно надуманны. Мы бы написали следующее:
...
Мы не собираемся говорить вам, что этот код легко читается. Мы вам скажем, что это самое сложное, что вам нужно понять 
на экзамене. Сравнивая его с предыдущим примером, вы можете видеть, что мы заменили counting() на mapping(). Так 
получилось, что метод mapping() принимает два параметра: функцию для значения и способ его дальнейшей группировки.

Вы можете увидеть сборщики, используемые со статическим импортом, чтобы сделать код короче. На экзамене может даже 
использоваться переменная var для возвращаемого значения и меньший отступ, чем мы использовали. Это означает, что вы 
можете увидеть что-то вроде этого:
...
Код делает то же самое, что и в предыдущем примере. Это означает, что важно распознавать имена сборщиков, поскольку у 
вас может не быть имени класса Collectors, которое могло бы привлечь к нему ваше внимание.

#### Teeing Collectors

Предположим, вы хотите вернуть две вещи. Как мы узнали, с потоками это проблематично, потому что у вас есть только один 
проход. Сводная статистика хороша, когда вам нужны эти операции. К счастью, вы можете использовать teeing() для возврата 
нескольких собственных значений.

Сначала определите тип возвращаемого значения. Здесь мы используем запись:

```
record Separations(String spaceSeparated, String commaSeparated) {}
```

Теперь пишем поток. Во время чтения обратите внимание на количество Collectors:
...
В этом коде есть три коллектора. Два из них предназначены для joining() и выдают значения, которые мы хотим вернуть. 
Третий метод — teeing(), который объединяет результаты в один объект, который мы хотим вернуть. Таким образом, Java 
доволен, потому что возвращается только один объект, а мы довольны, потому что нам не нужно проходить поток дважды.

## Резюме

Optional<T> может быть пустым или хранить значение. Вы можете проверить, содержит ли он значение, с помощью isPresent() 
и get() значение внутри. Вы можете вернуть другое значение с помощью orElse(T t) или создать исключение с помощью 
orElseThrow(). Есть даже три метода, которые принимают функциональные интерфейсы в качестве параметров: 
ifPresent(Consumer c), orElseGet(Supplier s) и orElseThrow(Supplier s). Существует три дополнительных типа примитивов:
OptionalDouble,OptionalInt иOptionalLong. У них есть методы getAsDouble(), getAsInt() и getAsLong() соответственно.

Потоковый конвейер состоит из трех частей. Требуется источник, и он создает данные в потоке. Может быть ноль или более 
промежуточных операций, которые не выполняются до тех пор, пока не будет запущена терминальная операция. Первым классом 
stream, который мы рассмотрели, был Stream<T>, который принимает общий аргумент T. Класс Stream<T> включает в себя 
множество полезных промежуточных операций, включая filter(), map(), flatMap() и sorted(). Примерами терминальных 
операций являются allMatch(), count() и forEach().

Помимо класса Stream<T>, существует три примитивных потока: DoubleStream, IntStream и LongStream. В дополнение к обычным 
методам Stream<T>, IntStream и LongStream имеют range() и rangeClosed(). Вызов range(1, 10) в IntStream и LongStream 
создает поток примитивов от 1 до 9. Напротив, вызов rangeClosed(1, 10) создает поток примитивов от 1 до 10. Потоки 
примитивов содержат математические операции, включая average(), max() и sum(). У них также есть summaryStatistics(), 
позволяющий получить множество статистических данных за один вызов.

Вы можете использовать Collector для преобразования потока в традиционную коллекцию. Вы даже можете сгруппировать поля, 
чтобы создать сложную карту в одной строке. Разбиение на разделы работает так же, как и группировка, за исключением 
того, что ключи всегда являются true и false. Разделенная карта всегда имеет два ключа, даже если значение для ключа 
пустое. Объединяющий коллектор позволяет объединять результаты двух других коллекторов.

Вам следует запомнить таблицу 10.6 и таблицу 10.7. По крайней мере, будьте в состоянии обнаружить несовместимости, 
например, различия типов. Наконец, помните, что потоки оцениваются лениво. Они принимают лямбда-выражения или ссылки на 
методы в качестве параметров, которые выполняются позже при запуске метода.

## Основы экзамена

**Напишите код, который использует Optional.**  Для создания Optional используется Optional.empty() или Optional.of(). 
При извлечении часто используются методы isPresent() и get(). Альтернативно существуют функциональные методы 
ifPresent() и orElseGet().

**Узнайте, какие операции вызывают выполнение потокового конвейера.** Промежуточные операции не выполняются до тех пор, 
пока не встретится терминальная операция. Если в конвейере нет операций терминала, Stream возвращается, но не 
выполняется. Примеры терминальных операций включают collect(), forEach(), min() и reduce().

**Определите, какие терминальные операции являются сокращениями.** При сокращении используются все элементы потока при 
определении результата. Сокращения, которые вам нужно знать, — это collect(), count(), max(), min() и reduce(). 
Изменяемая редукция по ходу собирается в один и тот же объект. Метод collect() является изменяемым сокращением.

**Напишите код для распространенных промежуточных операций.** Метод filter() возвращает фильтрацию Stream<T> по 
Predicate<T>. Метод map() возвращает поток, преобразуя каждый элемент типа T в другой тип R с помощью функции <T,R>. 
Метод flatMap() объединяет вложенные потоки в один уровень и удаляет пустые потоки.

**Сравните примитивные потоки со Stream<T>.**  Примитивные потоки полезны для выполнения общих операций с числовыми 
типами, включая такие статистические данные, как average(), sum() и т. д. Существует три примитивных класса потоков: 
DoubleStream, IntStream и LongStream. Также существует три примитивных Optional класса: OptionalDouble,OptionalInt и 
OptionalLong. За исключением BooleanSupplier, все они используют примитивы double, int или long.

**Преобразуйте примитивные типы потоков в другие примитивные типы потоков.** Обычно при сопоставлении вы просто 
вызываете метод map(). При изменении класса, используемого для потока, необходим другой метод. Чтобы преобразовать в
Stream, вы используете mapToObj(). Чтобы преобразовать в DoubleStream, вы используете mapToDouble(). Чтобы преобразовать 
в IntStream, вы используете mapToInt(). Чтобы преобразовать в LongStream, вы используете mapToLong().

**Используйте peek() для проверки потока.** Метод peek() — это промежуточная операция, часто используемая в целях 
отладки. Он выполняет ссылку на лямбда-метод или метод на входе и передает тот же ввод через конвейер следующему 
оператору. Это полезно для распечатки того, что проходит через определенную точку потока.

**Найдите поток.** Методы findFirst() и findAny() возвращают один элемент из потока в Optional. Методы anyMatch(), 
allMatch() и noneMatch() возвращают логическое значение. Будьте осторожны, потому что эти три могут зависнуть, если 
вызвать бесконечный поток с некоторыми данными. Все эти методы являются терминальными операциями.

**Сортировка потока.** Метод sorted() — это промежуточная операция, сортирующая поток. Существует две версии: подпись с 
нулевыми параметрами, которая сортируется с использованием естественного порядка сортировки, и подпись с одним 
параметром, которая сортируется с использованием этого компаратора в качестве порядка сортировки.

**Сравните groupingBy() и partitioningBy().** Метод groupingBy() — это терминальная операция, создающая карту. Ключи и 
типы возвращаемых значений определяются передаваемыми вами параметрами. Значения Map представляют собой коллекцию всех 
записей, сопоставленных с этим ключом. Метод partitioningBy() также возвращает Map. На этот раз ключи истинные и ложные. 
Значения снова представляют собой коллекцию совпадений. Если для этого логического значения нет совпадений, коллекция 
пуста.
