...
#### Таблица 12.16 - Опции, которые нужно знать для экзамена: jlink
...
В пути к классам появляется безымянный модуль. Как и автоматический модуль, это обычный JAR. В отличие от 
автоматического модуля, он находится в пути к классам, а не в пути к модулю. Это означает, что безымянный модуль 
рассматривается как старый код и как гражданин второго сорта по отношению к модулям.

Безымянный модуль обычно не содержит файла module-info.java. Если он есть, этот файл будет проигнорирован, поскольку он 
находится в пути к классам.

Безымянные модули не экспортируют пакеты в именованные или автоматические модули. Безымянный модуль может читать любые 
файлы JAR в пути к классам или модулю. Вы можете думать о безымянном модуле как о коде, который работает так же, как 
Java работала до появления модулей. Да, мы знаем, что наличие слова «модуль» в названии чего-то, что на самом деле не 
является модулем, сбивает с толку.

### Обзор типов модулей

На экзамене вы можете получить вопросы, сравнивающие три типа модулей. Пожалуйста, внимательно изучите Таблицу 12.17 и 
будьте готовы ответить на вопросы по этим пунктам в любой комбинации. Ключевой момент, который следует помнить, — это 
то, что код в пути к классам может получить доступ к пути к модулю. Напротив, код на пути к модулю не может читать из 
пути к классам.

#### Таблица 12.17 - Свойства типов модулей

|                                Свойство                                 |            Именованный            | Автоматический |           Безымянный            |  
|:-----------------------------------------------------------------------:|:---------------------------------:|:--------------:|:-------------------------------:|
|            Содержит ли модуль ______ файл module-info.java?             |                Да                 |      Нет       | Игнорируется, если присутствует |
|        Какие пакеты модуль ______ экспортирует в другие модули?         | Те, что в файле module-info.java. |   Все пакеты   |           Нет пакетов           |
| Доступен ли модуль ______ для чтения другими модулями на пути к модулю? |                Да                 |       Да       |               Нет               |
| Может ли модуль ______ читаться другими JAR-файлами в пути к классам?   |                Да                 |       Да       |               Да                |

## Миграция приложения

Многие приложения не были разработаны для использования системы модулей платформы Java, поскольку они были написаны до 
ее создания или решили не использовать ее. В идеале они, по крайней мере, разрабатывались как проекты, а не как большой 
ком грязи. В этом разделе представлен обзор стратегий перехода существующего приложения на использование модулей. 
Мы расскажем о заказе модулей, миграции снизу вверх, миграции сверху вниз и о том, как разделить существующий проект.

---

_Сценарий реального мира_

**Миграция ваших приложений на работе**

Экзамен проводится в воображаемой вселенной, где нет зависимостей с открытым исходным кодом, а приложения очень малы. 
Эти сценарии значительно облегчают изучение и обсуждение миграции. В реальном мире приложения имеют библиотеки, которые 
не обновлялись 10 и более лет, сложные графы зависимостей и всякие сюрпризы.

Обратите внимание, что вы можете использовать все функции Java 17 без преобразования приложения в модули (конечно, за 
исключением функций, описанных в этой главе о модулях!). Пожалуйста, убедитесь, что у вас есть причина для миграции и вы 
не считаете, что это необходимо.

Эта глава отлично расскажет вам, что вам нужно знать для экзамена. Однако он недостаточно готовит вас к преобразованию 
реальных приложений для использования модулей. Если вы оказались в такой ситуации, подумайте о том, чтобы прочитать 
«Систему модулей Java» Николая Парлога (Manning Publications, 2019).

---

### Определение порядка

Прежде чем мы сможем перенести наше приложение на использование модулей, нам необходимо знать, как структурированы 
пакеты и библиотеки в существующем приложении. Предположим, у нас есть простое приложение с тремя файлами JAR, как 
показано на рисунке 12.14. Зависимости между проектами образуют граф. Оба представления на рисунке 12.14 эквивалентны. 
Стрелки показывают зависимости, указывая от проекта, которому потребуется зависимость, к проекту, который делает ее 
доступной. На языке модулей стрелка будет идти от requires к exports.

#### Рис. 12.14 - Определение заказа



Правая часть диаграммы облегчает определение верха и низа, к которым относится миграция сверху вниз и снизу вверх. 
Проекты, не имеющие никаких зависимостей, находятся внизу. Проекты, у которых есть зависимости, находятся наверху.

В этом примере существует только один порядок сверху вниз, учитывающий все зависимости. Рисунок 12.15 показывает, что 
порядок не всегда уникален. Поскольку между двумя проектами нет стрелки, при выборе порядка миграции допускается 
любой порядок.


#### Рис. 12.15 - Определение порядка, если он не уникален


### Изучение стратегии миграции «снизу вверх»

Самый простой подход к миграции — миграция снизу вверх. Этот подход работает лучше всего, когда у вас есть возможность 
конвертировать любые файлы JAR, которые еще не являются модулями. Для миграции снизу вверх выполните следующие действия:

1. Выберите проект самого низкого уровня, который еще не был перенесен. (Помните, как мы упорядочили их по зависимостям в предыдущем разделе?)
2. Добавьте в этот проект файл module-info.java. Обязательно добавьте exports, чтобы предоставить доступ к любому пакету, используемому файлами JAR более высокого уровня. Кроме того, добавьте директиву requires для всех модулей, от которых зависит этот модуль.
3. Переместите этот только что перенесенный именованный модуль из пути к классам в путь к модулю.
4. Убедитесь, что все проекты, которые еще не были перенесены, остаются в пути к классам как безымянные модули.
5. Повторяйте действия с проектом следующего нижнего уровня, пока не закончите.

Вы можете увидеть, как эта процедура применяется для миграции трех проектов на рисунке 12.16. Обратите внимание, что 
каждый проект по очереди преобразуется в модуль.

При миграции снизу вверх вы приводите проекты нижнего уровня в хорошую форму. Это упрощает миграцию проектов верхнего 
уровня в конце. Это также поощряет осторожность в отношении того, что подвергается воздействию.

Во время миграции у вас есть смесь именованных и неименованных модулей. Именованные модули — это перенесенные модули 
нижнего уровня. Они находятся на пути к модулю, и им не разрешен доступ к каким-либо безымянным модулям.

#### Рис. 12.16 - Миграция снизу вверх



Безымянные модули находятся в пути к классам. Они могут получить доступ к файлам JAR как по пути к классам, так и по 
пути к модулю.

### Изучение стратегии миграции «сверху вниз»

Стратегия миграции сверху вниз наиболее полезна, когда у вас нет контроля над каждым файлом JAR, используемым вашим 
приложением. Например, предположим, что другая команда владеет одним проектом. Они просто слишком заняты, чтобы 
мигрировать. Вы бы не хотели, чтобы эта ситуация задержала всю вашу миграцию.

Для миграции сверху вниз выполните следующие действия:

1. Поместите все проекты на путь к модулю.
2. Выберите проект самого высокого уровня, который еще не был перенесен.
3. Добавьте в этот проект файл module-info.java, чтобы преобразовать автоматический модуль в именованный модуль. Опять же, не забудьте добавить любые директивы exports или requires. Вы можете использовать автоматическое имя других модулей при написании директивы requires, поскольку большинство проектов в пути к модулю еще не имеют имен.
4. Повторяйте действия с проектом следующего уровня, пока не закончите.

Вы можете увидеть, как эта процедура применяется для переноса трех проектов на рисунке 12.17. Обратите внимание, что 
каждый проект по очереди преобразуется в модуль.

#### Рис. 12.17 - Миграция сверху вниз


При нисходящей миграции вы признаете, что все зависимости более низкого уровня не готовы, но что вы хотите сделать само 
приложение модулем.

Во время миграции у вас есть набор именованных модулей и автоматических модулей. Названные модули - это модули более 
высокого уровня, которые были перенесены. Они находятся на пути к модулю и имеют доступ к автоматическим модулям. 
Автоматические модули также находятся на пути к модулю.

В таблице 12.18 представлено то, что вам нужно знать о двух основных стратегиях миграции. Убедитесь, что вы хорошо 
это знаете.

#### Таблица 12.18 - Сравнение миграционных стратегий

|                 Категория                 |             Снизу вверх             | Сверху вниз                             |  
|:-----------------------------------------:|:-----------------------------------:|-----------------------------------------|
| Проект, который зависит от всех остальных | Неназванный модуль в пути к классу  | Именованный модуль в пути к модулю      |
|     Проект, не имеющий зависимостей       | Именованный модуль в пути к модулю  | Автоматический модуль на пути к модулю  |

### Разбиение большого проекта на модули

Для подготовки к экзамену вам необходимо понять основной процесс разделения большого проекта на модули. Конечно, вам не 
дадут большого проекта. В конце концов, места для того, чтобы задать вопрос, не так уж много. К счастью, для небольшого 
проекта процесс такой же.

Предположим, вы начинаете с приложения, содержащего несколько пакетов. Первый шаг - разбить их на логические группировки 
и провести зависимости между ними. На рисунке 12.18 показана декомпозиция воображаемой системы. Обратите внимание, что 
слева и справа расположены семь пакетов. Модулей меньше, потому что некоторые пакеты используют один и тот же модуль 
совместно.

#### Рис. 12.18 - Первая попытка разложения


С этим разложением есть проблема. Ты это видишь? Модульная система платформы Java не допускает циклических зависимостей. 
Циклическая зависимость, или порочный круг, - это когда две вещи прямо или косвенно зависят друг от друга. Если в 
zoo.tickets.delivery для модуля доставки требуется модуль zoo.tickets.discount, zoo.tickets.discount не может требовать
zoo.tickets.delivery модуль.

Теперь, когда мы знаем, что разложение на рис. 12.18 не сработает, что мы можем с этим поделать? Распространенным 
методом является введение другого модуля. Этот модуль содержит код, который совместно используется двумя другими 
модулями. На рисунке 12.19 показаны новые модули без каких-либо циклических зависимостей. Обратите внимание на новый 
модуль zoo.tickets.etech. Мы создали новые пакеты для размещения в этом модуле. Это позволяет разработчикам поместить 
туда общий код и разорвать зависимость. Больше никаких циклических зависимостей!

### Не удалось скомпилировать с циклической зависимостью

Чрезвычайно важно понимать, что Java не позволит вам компилировать модули, которые имеют циклические зависимости. В 
этом разделе мы рассмотрим пример, приводящий к этой ошибке компилятора.

#### Рис. 12.19 - Удаление циклических зависимостей


Рассмотрим модуль zoo.butterfly, описанный здесь:
...
Мы пока не можем скомпилировать это, так как сначала нам нужно собрать zoo.caterpillar. Ведь наша бабочка requires этого. 
Теперь посмотрим на zoo.caterpillar:
...
Мы пока не можем это скомпилировать, так как сначала нам нужно собрать Zoo.butterfly. Ой-ой! Сейчас у нас тупиковая 
ситуация. Ни один модуль не может быть скомпилирован. Это наша проблема циклической зависимости в действии.

Это одно из преимуществ модульной системы. Это не позволяет вам писать код, имеющий циклическую зависимость. 
Такой код даже не скомпилируется!

Возможно, вам интересно, что произойдет, если задействовать три модуля. Предположим, для модуля ballA требуется модуль 
bulB, а для bulB требуется модуль ballC. Может ли модуль ballC требовать модуль ballA? Нет. Это создало бы циклическую 
зависимость. Не верите нам? Попробуй нарисовать это. Вы можете водить карандашом по кругу от ballA до ballB, от ballC 
до ballA to ....что ж, вы поняли идею. В воздухе просто слишком много шаров!

_Java по-прежнему позволит вам иметь циклическую зависимость между пакетами внутри модуля. Это гарантирует, что у вас 
нет циклической зависимости между модулями._

## Резюме

Модульная система платформы Java организует код на более высоком уровне, чем пакеты. Каждый модуль содержит один или 
несколько пакетов и module-info.java файл. Модуль java.base является наиболее распространенным и автоматически 
предоставляется всем модулям в качестве зависимости.

В процессе компиляции и запуска модулей используется --module-path, также известный как -p. При запуске модуля 
используется параметр --module, также известный как -m. Класс для запуска указывается в формате moduleName/className.

Файл объявления модуля поддерживает ряд директив. Директива exports указывает, что пакет должен быть доступен вне модуля. 
При желании он может ограничить этот экспорт определенным пакетом. Директива requires используется, когда модуль зависит 
от кода в другом модуле. Кроме того, requires transitive может использоваться, когда все модули, которым требуется один 
модуль, всегда должны требовать другой. Директивы provides и uses используются при совместном использовании сервиса и 
его потреблении. Наконец, директива opens используется для разрешения доступа через отражение.

Для описания содержимого модуля можно использовать как команды java, так и команды jar. Команда java может дополнительно 
перечислять доступные модули и показывать разрешение модуля. Команда jdeps выводит информацию об используемых пакетах в 
дополнение к информации на уровне модуля. Команда jmod используется при работе с файлами, которые не соответствуют 
требованиям к JAR. Команда link создает уменьшенный образ среды выполнения Java.

Существует три типа модулей. Именованные модули содержат файл module-info.java и находятся в пути к модулю. Они могут 
считываться только из пути к модулю. Автоматические модули также находятся в пути к модулю, но еще не были модулированы. 
У них может быть автоматическое имя модуля, заданное в манифесте. Неназванные модули находятся в пути к классу.

Две наиболее распространенные стратегии миграции - это миграция "сверху вниз" и "снизу вверх". Нисходящая миграция 
запускает миграцию модуля с наибольшим количеством зависимостей и размещает все остальные модули на пути к модулю. 
Восходящая миграция запускает миграцию модуля без зависимостей и перемещает по одному модулю в путь к модулю за раз. 
Обе эти стратегии требуют обеспечения того, чтобы у вас не было никаких циклических зависимостей, поскольку модульная 
система платформы Java не позволяет компилировать циклические зависимости.

## Основы экзамена

**Создайте module-info.java файлы.** Поместите module-info.java файл в корневом каталоге модуля. Знать, как кодировать 
директивы exports, requires, provides и uses. Кроме того, ознакомьтесь с директивой opens.

**Используйте операции командной строки с модулями.** Команда java может описывать модуль, перечислять доступные модули 
или показывать разрешение модуля. Команда jar может описывать модуль аналогично тому, как это делает команда java. 
Команда jdeps выводит подробные сведения о модуле и пакетах. Команда jmod предоставляет различные режимы для работы с 
файлами JMOD, а не с JAR-файлами. Команда link создает пользовательские Java-образы.

**Определите три типа модулей.** Именованные модули — это JAR-файлы, которые были разделены на модули. Безымянные модули 
не были модульными. Автоматические модули находятся между ними. Они находятся на пути к модулю, но не имеют файла 
mod-info.java.

**Список встроенных модулей JDK.** Модуль java.base доступен для всех модулей. В состав JDK входит около 20 других 
модулей, имена которых начинаются с java.* , и около 30, начинающиеся с jdk.*.

**Объясните миграцию сверху вниз и снизу вверх.** При миграции сверху вниз все файлы JAR помещаются на путь к модулю, 
что делает их автоматическими модулями при миграции сверху вниз. Миграция снизу вверх оставляет все JAR-файлы в пути к 
классам, делая их безымянными модулями при миграции снизу вверх.

**Различайте четыре основные части услуги.** Интерфейс поставщика услуг объявляет интерфейс, который должна 
реализовать служба. Локатор службы ищет службу, и потребитель вызывает службу. Наконец, поставщик услуг реализует 
услугу.
