...
а затем повторная сборка результатов. Чем более параллельна декомпозиция, тем выше повышение производительности при 
использовании параллельных потоков.

Давайте попробуем. Во-первых, давайте определим функцию многократного использования, которая «действительно работает», 
просто подождав пять секунд.
...
Мы можем представить, что в реальном приложении эта работа может включать вызов базы данных или чтение файла. Теперь 
давайте воспользуемся этим методом с последовательным потоком.
...
Как вы думаете, что выведет этот код, если он будет выполнен как часть метода main()? Давайте посмотрим:
...
Как и следовало ожидать, результаты упорядочены и предсказуемы, поскольку мы используем последовательный поток. 
Обработка всех пяти результатов по одному также заняла около 25 секунд. Что произойдет, если мы заменим строку 12 
строкой, в которой используется parallelStream()? Ниже приведен пример вывода:
...
Как видите, результаты больше не упорядочены и не предсказуемы. Операции map() и forEach() в параллельном потоке 
эквивалентны отправке нескольких лямбда-выражений Runnable исполнителю объединенного потока с последующим ожиданием 
результатов.

А что насчет необходимого времени? В этом случае в нашей системе было достаточно процессоров для одновременного 
выполнения всех задач. Если вы запустили тот же код на компьютере с меньшим количеством процессоров, он может вывести 
10 секунд, 15 секунд или какое-то другое значение. Ключевым моментом является то, что мы написали наш код так, чтобы 
использовать преимущества параллельной обработки, когда она доступна, поэтому наша работа выполнена.

---

**Результаты заказа**

Если ваша потоковая операция должна гарантировать упорядоченность, и вы не уверены, является ли она последовательной или 
параллельной, вы можете заменить строку 14 строкой, в которой используется forEachOrdered():

```
14:    .forEachOrdered(s -> System.out.print(s + " "));
```

Это выводит результаты в том порядке, в котором они определены в потоке:
...
Хотя мы потеряли часть прироста производительности от использования параллельного потока, наша операция map() все еще 
может использовать преимущества параллельного потока.

---


### Обработка параллельных сокращений

Помимо потенциального повышения производительности и изменения порядка операций, использование параллельных потоков 
может повлиять на то, как вы пишете свое приложение. Параллельное сокращение — это операция сокращения, применяемая к 
параллельному потоку. Результаты параллельного сокращения могут отличаться от ожидаемых при работе с 
последовательными потоками.

### Выполнение задач на основе заказов

Поскольку порядок в параллельных потоках не гарантируется, такие методы, как findAny() для параллельных потоков, могут 
привести к неожиданному поведению. Рассмотрим следующий пример:
...
JVM выделяет несколько потоков и возвращает значение первого из них, возвращающего результат, который может быть 4, 2 и 
т.д. Хотя ни последовательный, ни параллельный поток не гарантированно возвращают первое значение, последовательный 
поток часто это делает. При параллельном потоке результаты, скорее всего, будут более случайными.

А как насчет операций, учитывающих порядок, таких как findFirst(), limit() и skip()? Порядок по-прежнему сохраняется, 
но производительность параллельного потока может снизиться в результате того, что задача параллельной обработки 
вынуждена координировать все свои потоки синхронизированным образом.

Плюсом является то, что результаты упорядоченных операций в параллельном потоке будут согласовываться с результатами 
последовательного потока. Например, вызов skip(5).limit(2).findFirst() вернет один и тот же результат для упорядоченных 
последовательных и параллельных потоков.

---

_Сценарий реального мира_

**Создание неупорядоченных потоков**

Все потоки, с которыми вы работали, по умолчанию считаются упорядоченными. Можно создать неупорядоченный поток из 
упорядоченного потока, аналогично тому, как вы создаете параллельный поток из последовательного потока.

```
List.of(1,2,3,4,5,6).stream().unordered();
```

Этот метод не меняет порядок элементов; он просто сообщает JVM, что если применяется потоковая операция на основе 
порядка, то порядок можно игнорировать. Например, вызов skip(5) в неупорядоченном потоке пропустит любые 5 элементов, 
не обязательно первые 5, необходимые в упорядоченном потоке.

Для последовательных потоков использование неупорядоченной версии не имеет никакого эффекта. А вот на параллельных 
потоках результаты могут значительно повысить производительность.

```
List.of(1,2,3,4,5,6).stream().unordered().parallel();
```

Несмотря на то, что неупорядоченные потоки не будут на экзамене, если вы разрабатываете приложения с параллельными 
потоками, вы должны знать, когда применять неупорядоченный поток для повышения производительности.

---


### Объединение результатов с помощью reduce()

Как вы узнали из главы 10, потоковая операция reduce() объединяет поток в единый объект. Напомним, что первый параметр 
метода reduce() называется идентификатором, второй параметр называется накопителем, а третий параметр называется 
объединителем. Ниже приведена сигнатура для метода:
...
Мы можем объединить список значений char, используя метод reduce(), как показано в следующем примере:
...
_Присвоение имен переменным в этом примере потока не случайно. Мы использовали c для char, тогда как s1, s2 и s3 
являются строковыми значениями._

В параллельных потоках метод reduce() работает путем применения сокращения к парам элементов в потоке для создания 
промежуточных значений, а затем объединения этих промежуточных значений для получения конечного результата. 
Другими словами, в последовательном потоке wolf создается по одному символу за раз. В параллельном потоке создаются 
промежуточные значения wo и lf, которые затем объединяются.

С параллельными потоками нам теперь приходится беспокоиться о порядке. Что, если элементы строки объединяются в 
неправильном порядке для получения wlfo или flwo? Stream API предотвращает эту проблему, по-прежнему позволяя 
обрабатывать потоки параллельно, при условии, что вы следуете одному простому правилу: убедитесь, что накопитель и 
объединитель выдают один и тот же результат независимо от порядка их вызова.

_Хотя это не входит в сферу применения экзамена, накопитель и объединитель должны быть ассоциативными, не создавать 
помех и не иметь состояния. Не паникуйте; вам не нужно знать продвинутые математические термины для экзамена!_

Хотя требования к входным аргументам метода reduce() справедливы как для последовательных, так и для параллельных 
потоков, вы, возможно, не заметили никаких проблем в последовательных потоках, поскольку результат всегда был 
упорядочен. Однако при использовании параллельных потоков порядок больше не гарантируется, и любой аргумент, нарушающий 
эти правила, с гораздо большей вероятностью приведет к побочным эффектам или непредсказуемым результатам.

Давайте рассмотрим пример с использованием проблемного накопителя. В частности, порядок имеет значение при вычитании 
чисел; поэтому следующий код может выводить разные значения в зависимости от того, используете ли вы последовательный 
или параллельный поток. В этих примерах мы можем опустить параметр объединителя, поскольку накопитель можно использовать, 
когда промежуточные типы данных совпадают.
...
Он может выводить значение -21, 3 или какое-либо другое значение.

Вы можете увидеть другие проблемы, если мы используем параметр идентификатора, который на самом деле не является 
значением идентификатора. Например, что вы ожидаете от следующего кода?
...
В последовательном потоке он печатает Xwolf, а в параллельном потоке результат — XwXoXlXf. В рамках параллельного 
процесса идентификатор применяется к нескольким элементам потока, что приводит к получению весьма неожиданных данных.

--- 

**Выбор метода reduce()**

Хотя версии reduce() с одним и двумя аргументами поддерживают параллельную обработку, рекомендуется использовать версию 
reduce() с тремя аргументами при работе с параллельными потоками. Предоставление явного метода объединения позволяет 
виртуальной машине JVM более эффективно распределять операции в потоке.

---

### Объединение результатов с помощью collect()

Как и reduce(), Stream API включает версию collect() с тремя аргументами, которая принимает операторы аккумулятора и 
объединителя вместе с оператором поставщика вместо идентификатора.
...
Также, как и reduce(), операции накопления и объединения должны иметь возможность обрабатывать результаты в любом 
порядке. Таким образом, версия collect() с тремя аргументами может быть выполнена как параллельное сокращение, как 
...
Напомним, что элементы в ConcurrentSkipListSet сортируются в соответствии с их естественным порядком. Вы должны 
использовать параллельную коллекцию для объединения результатов, гарантируя, что результаты параллельных потоков не 
вызывают ConcurrentModificationException.

Выполнение параллельных сокращений с помощью сборщика требует дополнительных соображений. Например, если коллекция, в 
которую вы вставляете, представляет собой упорядоченный набор данных, такой как список, элементы в результирующей 
коллекции должны быть в том же порядке, независимо от того, используете ли вы последовательный или параллельный поток. 
Однако это может снизить производительность, поскольку некоторые операции не могут быть выполнены параллельно.

### Выполнение параллельного сокращения на коллекторе

Хотя мы рассматривали интерфейс Collector в главе 10, мы не вдавались в подробности о его свойствах. Каждый экземпляр 
Collector определяет метод characteristics(), который возвращает набор атрибутов Collector.Characteristics. При 
использовании Collector для выполнения параллельного сокращения должен выполняться ряд свойств. В противном случае 
операция collect() будет выполняться однопоточным способом.

Требования к параллельному сокращению с помощью collect()

+ Поток параллелен.
+ Параметр операции collect() имеет характеристику Characteristics.CONCURRENT.
+ Либо поток неупорядочен, либо коллектор имеет характеристику Characteristics.UNORDERED.

Например, хотя Collectors.toSet() имеет характеристику UNORDERED, она не имеет характеристики CONCURRENT. Следовательно, 
следующее не является параллельным сокращением даже с параллельным потоком:

```
parallelStream.collect(Collectors.toSet());  // Not a parallel reduction
```

Класс Collectors включает в себя два набора статических методов для извлечения коллекторов: toConcurrentMap() и 
groupingByConcurrent(), оба из которых являются UNORDERED и CONCURRENT. Эти методы создают экземпляры Collector, 
способные эффективно выполнять параллельные сокращения. Как и их непараллельные аналоги, существуют перегруженные версии, 
принимающие дополнительные аргументы.

Вот переписанный пример из главы 10 для использования параллельного потока и параллельного сокращения:
...
Мы используем ссылку ConcurrentMap, хотя фактически возвращаемый класс, скорее всего, ConcurrentHashMap. Конкретный 
класс не гарантирован; это будет просто класс, реализующий интерфейс ConcurrentMap.

Наконец, мы можем переписать наш пример groupingBy() из главы 10, чтобы использовать параллельный поток и 
...
Как и прежде, возвращаемый объект можно присвоить ссылке ConcurrentMap.

---

_Сценарий реального мира_

**Избегание потоков с отслеживанием состояния**

Побочные эффекты могут проявляться в параллельных потоках, если ваши лямбда-выражения сохраняют состояние. 
Лямбда-выражение с сохранением состояния — это выражение, результат которого зависит от любого состояния, которое 
может измениться во время выполнения конвейера. Например, следующий метод, который отфильтровывает четные числа, имеет 
состояние:
...
Допустим, этот метод выполняется с последовательным потоком:
...
Отлично, результаты находятся в том же порядке, в котором они были введены. А что, если в параллельном потоке пройдет 
кто-то еще?
...
О, нет: наши результаты больше не соответствуют порядку ввода! Проблема в том, что наше лямбда-выражение имеет состояние 
и изменяет список, находящийся за пределами нашего потока. Мы можем исправить это решение, переписав нашу потоковую 
операцию так, чтобы она не сохраняла состояние:
...
Этот метод обрабатывает поток, а затем собирает все результаты в новый список. Он дает один и тот же упорядоченный 
результат как для последовательных, так и для параллельных потоков. Настоятельно рекомендуется избегать операций с 
отслеживанием состояния при использовании параллельных потоков, чтобы устранить любые потенциальные побочные эффекты 
данных. Фактически, их следует избегать в последовательных потоках, поскольку это ограничивает способность кода 
когда-нибудь воспользоваться преимуществами распараллеливания.
...
В этой главе вы познакомились с потоками и обрисовали некоторые ключевые концепции параллелизма, которые вам необходимо 
знать для сдачи экзамена (и чтобы стать лучшим разработчиком программного обеспечения!). Вы должны знать, как создавать 
и определять работу потока с помощью экземпляра Runnable, а также как приостанавливать и прерывать поток. При работе с
Concurrency API вы также должны знать, как создавать потоки с использованием Callable лямбда-выражений.

На этом этапе вы должны знать, как одновременно выполнять задачи с помощью ExecutorService, как профессионал. Вы также 
должны знать, какие экземпляры ExecutorService доступны, включая запланированные и объединенные службы.

Потокобезопасность заключается в защите данных от повреждения несколькими потоками, изменяющими их одновременно. 
Java предлагает множество инструментов для обеспечения безопасности данных, включая атомарные классы, 
синхронизированные методы/блоки, платформу блокировки и CyclicBarrier. Concurrency API также включает в себя множество 
классов коллекций, которые обеспечивают многопоточный доступ. Вы должны быть знакомы с параллельными коллекциями, 
включая классы CopyOnWrite, которые создают новую базовую структуру каждый раз при изменении базовой коллекции.

При одновременной обработке задач могут возникнуть различные потенциальные проблемы с потоками. Взаимная блокировка, 
голодание и активная блокировка могут привести к зависанию программ, а условия гонки могут привести к непредсказуемым 
данным. Для сдачи экзамена вам нужно знать только основную теорию, лежащую в основе этих понятий. Однако в 
профессиональной разработке программного обеспечения обнаружение и решение подобных проблем является ценным навыком.

Наконец, мы обсудили параллельные потоки и показали, как использовать их для параллельной декомпозиции и сокращения. 
Параллельные потоки могут значительно повысить производительность вашего приложения. Они также могут привести к 
неожиданным результатам, поскольку обработка больше не заказывается. Не забывайте избегать лямбда-выражений с 
сохранением состояния, особенно при работе с параллельными потоками.

## Основы экзамена

**Уметь писать потокобезопасный код.** Потокобезопасность заключается в защите общих данных от одновременного доступа. 
Монитор можно использовать, чтобы гарантировать, что только один поток обрабатывает определенный участок кода одновременно. 
В Java мониторы могут быть реализованы с помощью синхронизированного блока или метода или с использованием экземпляра 
Lock. ReentrantLock имеет ряд преимуществ перед использованием синхронизированного блока, включая возможность проверять, 
доступна ли блокировка, не блокируя ее, а также поддерживать справедливое получение блокировок. Для достижения 
синхронизации два или более потоков должны координировать свои действия на одном и том же общем объекте.

**Уметь применять атомарные классы.** Атомарная операция — это операция, которая выполняется без вмешательства другого
потока. Concurrency API включает набор атомарных классов, которые аналогичны примитивным классам, за исключением того, 
что они гарантируют, что операции над ними выполняются атомарно. Знайте разницу между атомарной переменной и переменной, 
отмеченной модификатором volatile.

**Создавайте параллельные задачи с помощью службы исполнителя потоков, используя Runnable и Callable.**
ExecutorService создает один поток или пул потоков и управляет ими. Экземпляры Runnable и Callable могут быть отправлены 
исполнителю потока и будут завершены с использованием доступных потоков в службе. Callable отличается от Runnable тем, 
что Callable возвращает общий тип данных и может генерировать проверенное исключение. ScheduledExecutorService можно 
использовать для планирования задач с фиксированной скоростью или с фиксированным интервалом между выполнениями.

**Уметь использовать параллельные классы коллекций.** Concurrency API включает в себя множество классов коллекций, 
включающих встроенную поддержку многопоточной обработки, например ConcurrentHashMap. Он также включает класс 
CopyOnWriteArrayList, который создает копию своей базовой структуры списка каждый раз, когда она изменяется, и полезен 
в средах с высокой степенью параллелизма.

**Выявление потенциальных проблем с потоками.** Взаимная блокировка, голодание и активная блокировка — это три проблемы 
с потоками, которые могут возникнуть и привести к тому, что потоки никогда не завершат свою задачу. Взаимная блокировка 
возникает, когда два или более потоков блокируются навсегда. «Голод» возникает, когда одному потоку постоянно отказывают 
в доступе к общему ресурсу. Livelock — это форма голодания, при которой два или более потоков активны, но концептуально 
заблокированы навсегда. Наконец, когда два потока выполняются одновременно, возникают условия гонки, что приводит к 
неожиданному результату.

**Поймите влияние использования параллельных потоков.** Stream API позволяет легко создавать параллельные потоки. 
Использование параллельного потока может привести к неожиданным результатам, поскольку порядок операций может оказаться 
непредсказуемым. Некоторые операции, такие как reduce() и collect(), требуют особого внимания для достижения оптимальной
производительности при применении к параллельному потоку.
