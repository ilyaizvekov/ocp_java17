...
Основываясь на всем, что вы узнали до сих пор о числовом преобразовании и кастинге, вы понимаете, почему 
последняя строка этого утверждения не будет компилироваться? Как вы, возможно, помните, short значения 
автоматически преобразуются в int при применении любого арифметического оператора, при этом результирующее значение 
имеет тип int. Попытка присвоить короткой short значение int приводит к ошибке компилятора, поскольку Java 
...
short mouse = 10;
short hamster = 3;
short capybara = (short)mouse * hamster; // НЕ КОМПИЛИРУЕТСЯ
```

Итак, что происходит в последней строке? Помните, мы говорили, что приведение типов — это унарная операция? Это 
означает, что приведение в последней строке применяется к mouse и только к mouse. После завершения приведения оба 
...
```
short capybara = 1 + (short)(mouse * hamster); // НЕ КОМПИЛИРУЕТСЯ
```
...
### Приведение значений против переменных

Возвращаясь к нашему третьему числовому правилу продвижения: компилятор не требует приведения приведения при работе с литеральными значениями, 
которые соответствуют типу данных. Рассмотрим эти примеры:
...
```
short boots = 2 + hat; // НЕ КОМПИЛИРУЕТСЯ
byte gloves = 7 * 100; // НЕ КОМПИЛИРУЕТСЯ
```

Первый оператор не компилируется, потому что hat — это переменная, а не значение, и оба операнда автоматически 
...
camel = camel * giraffe;     // Простой оператор присваивания
camel *= giraffe;            // Составной оператор присваивания
```
...
long goat = 10;
int sheep = 5;
sheep = sheep * goat;  // НЕ КОМПИЛИРУЕТСЯ
```
...
### Возвращаемое значение операторов присваивания
...
```
boolean monkey = true == 3;       // НЕ КОМПИЛИРУЕТСЯ
boolean ape = false != "Grape";   // НЕ КОМПИЛИРУЕТСЯ
boolean gorilla = 10.2 == "Koko"; // НЕ КОМПИЛИРУЕТСЯ
```
...
присваивания, само присваивание имеет значение присваивания. Следовательно, polar также присваивается значение 
...
Несмотря на то, что все переменные указывают на одну и ту же информацию о файле, только две ссылки, tuesday и wednesday, 
...
должны быть в состоянии ответить на вопросы, указывающие на то, что monday и tuesday — это два отдельных объекта, 
...
подклассами. Например, представьте, что у нас есть метод, который открывает зоопарк и печатает время. В 
...
Теперь мы хотим, чтобы метод добавлял часы в конец вывода, если значение представляет собой целое число, например 
...
```
public void openZoo(Number time) {
    if(time instanceof String) // НЕ КОМПИЛИРУЕТСЯ
       System.out.print(time);
}
```
...
### null и оператор instanceof

Что произойдет, если вы вызовете instanceof для null переменной? Для экзамена вы должны знать, что вызов 
instanceof для null литерала или null ссылки всегда возвращает false.
...
### Избегание исключения NullPointerException

Более распространенным примером использования условных операторов является проверка null объектов перед 
...
```
if(duck!=null & duck.getAge()<5) { // Мог бы кинуть NullPointerException
// Сделайте что-нибудь
}
```
...
### Проверка на отсутствие побочных эффектов
...
Поскольку rabbit >= 6 true, оператор инкремента в правой части выражения никогда не вычисляется, поэтому на выходе 
будет 6.

## Принятие решений с тернарным оператором
...
---
#### Тернарное выражение и невыполненные побочные эффекты
...
На экзамене будьте осторожны с любым вопросом, который включает тернарное выражение, в котором переменная изменяется 
