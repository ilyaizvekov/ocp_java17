...
Альтернативно, переменные, объявленные перед циклом for и которым присвоено значение в блоке инициализации, могут 
...
На второй итерации внешнего цикла внутренний do/while будет выполнен один раз, даже несмотря на то, что hungryHippopotamus уже не превышает 5. Как вы, возможно, помните, операторы do/while всегда выполняют тело по крайней мере один раз. Это уменьшит значение до 1, которое будет дополнительно уменьшено оператором декремента во внешнем цикле до 0. Как только значение достигнет 0, внешний цикл завершится. В результате код выведет следующее: 3, 0,


Примеры в оставшейся части этого раздела включают множество вложенных циклов. На экзамене вы также столкнетесь с вложенными циклами, поэтому чем больше вы будете с ними практиковаться, тем лучше вы будете подготовлены.
...
Вы можете заметить, что синтаксис оператора continue аналогичен синтаксису оператора break. На самом деле операторы идентичны в том, как они используются, но с разными результатами. В то время как оператор break передает управление закрывающему оператору, оператор continue передает управление логическому выражению, которое определяет, должен ли цикл продолжаться. Другими словами, он завершает текущую итерацию цикла. Кроме того, как и оператор break, оператор continue применяется к ближайшему выполняемому внутреннему циклу, используя необязательные операторы меток для переопределения этого поведения.

or

Вы можете заметить, что синтаксис инструкции continue повторяет синтаксис инструкции break. На самом деле, инструкции идентичны в том, как они используются, но с разными результатами. В то время как оператор break передает управление заключающему оператору, оператор continue передает управление логическому выражению, которое определяет, должен ли цикл продолжаться. Другими словами, это завершает текущую итерацию цикла. Также, как и оператор break, оператор continue применяется к ближайшему выполняемому внутреннему циклу, используя необязательные операторы label для переопределения этого поведения.

Давайте посмотрим на пример. Представьте, что у нас есть смотритель зоопарка, который должен почистить первого леопарда в каждой из четырех конюшен, но полностью пропустить конюшню b.
...
С такой структурой, как определено, цикл будет возвращать управление родительскому циклу в любое время, когда первое значение равно b или второе значение равно 2. При первом, третьем и четвертом выполнении внешнего цикла внутренний цикл печатает оператор точно один раз, а затем завершается в следующем внутреннем цикле, когда leopard равен 2. При втором выполнении внешнего цикла внутренний цикл немедленно завершается, ничего не печатая, так как b встречается сразу. Печатается следующее:

or

С определенной структурой цикл будет возвращать управление родительскому циклу каждый раз, когда первое значение равно b или второе значение равно 2. При первом, третьем и четвертом выполнении внешнего цикла внутренний цикл печатает инструкцию ровно один раз, а затем завершает следующий внутренний цикл, когда leopard равен 2. При втором выполнении внешнего цикла внутренний цикл немедленно завершается, ничего не печатая, поскольку b встречается сразу. Печатается следующее:
...
Это соответствует тому, что смотритель зоопарка чистит всех леопардов, кроме тех, которые помечены как 2 или находятся в стойле в. Тогда результат будет следующим:

or

Это соответствует чистке смотрителем зоопарка всех леопардов, кроме тех, которые помечены цифрой 2 или в хлеву b. Результат будет следующим:
...
Учитывая, что эта книга не должна быть вашим первым знакомством с программированием, мы надеемся, что вы познакомились с методами, содержащими операторы возврата. Несмотря на это, мы подробно расскажем, как проектировать и создавать методы, которые их используют, в главе 5 «Методы».

Однако сейчас вы должны быть знакомы с идеей о том, что создание методов и использование операторов return можно использовать в качестве альтернативы использованию меток и операторов break. Например, взгляните на эту переписанную версию нашего предыдущего класса FindInMatrix:
...
**Изучите операторы switch и их правильное использование.**
