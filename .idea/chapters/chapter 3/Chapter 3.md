...
2. Добавление нескольких условий в оператор for
...
3. Повторное объявление переменной в блоке инициализации

```
int x = 0;
for (int x = 4; x < 5; x++) // НЕ КОМПИЛИРУЕТСЯ
 System.out.print(x + " ");
```
...
4. Использование несовместимых типов данных в блоке инициализации

```
int x = 0;
for (long y = 0, int z = 4; x < 5; x++) // НЕ КОМПИЛИРУЕТСЯ
 System.out.print(y + " ");
```
...
5. Использование переменных цикла вне цикла

```
for (long y = 0, x = 4; x < 5 && y < 10; x++, y++)
 System.out.print(y + " ");
System.out.print(x); // НЕ КОМПИЛИРУЕТСЯ
```
...
```
String birds = "Jay";
for (String bird : birds) // НЕ КОМПИЛИРУЕТСЯ
   System.out.print(bird + " ");
 
String[] sloths = new String[3];
for (int sloth : sloths) // НЕ КОМПИЛИРУЕТСЯ
...
## Управление потоком с помощью ветвления
...
Как вы видели при работе с операторами switch, оператор break передает поток управления заключающему
...
применяется к ближайшему выполняемому внутреннему циклу, используя необязательные операторы меток для 
переопределения этого поведения.

Давайте взглянем на пример. Представьте, что у нас есть смотритель зоопарка, который должен чистить первого леопарда в каждом из четырех вольеров, но полностью пропускает вольер b.
...
Это соответствует тому, что смотритель зоопарка чистит все вольеры леопардов, кроме тех, которые помечены как 2 или находятся 
в вольере b. Тогда результат будет следующим:
...
с методами, содержащие оператор return. Несмотря на это, мы подробно расскажем, как проектировать и создавать 
...
Мы завершаем этот раздел таблицей 3.1, которая поможет напомнить вам, когда в Java разрешены операторы меток, 
