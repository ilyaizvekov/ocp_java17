...
+ Используйте примитивы и классы-оболочки, включая Math API, круглые скобки, расширение и приведение типов для вычисления арифметических и логических выражений
...
#### Переопределение toString(), equals(Object), and hashCode()
...
В некоторых странах соблюдается летнее время. Здесь часы дважды в год переводятся на час, чтобы лучше использовать 
солнечный свет. Не все страны участвуют, а те, которые участвуют, используют разные выходные для перестройки. На 
экзамене вам нужно работать только с переходом на летнее время в США, и это то, что мы описываем здесь.

Этот вопрос позволит вам узнать, выпадает ли указанная дата/время на выходные, когда часы должны быть переведены. 
Если это не указано в вопросе, можно считать, что это обычные выходные. Действие по перемещению часов вперед или назад 
происходит в 2 часа ночи, что выпадает на очень раннее воскресное утро.

На рисунке 4.10 показано, что происходит с часами. Когда мы переводим наши часы в марте, время перемещается вперед с 1:59 утра до 3:00 утра. Когда мы переводим наши часы в ноябре, время возвращается назад, и мы дважды переживаем час с 1:00 утра до 1:59 утра. Дети усваивают это как “Весной прыгай вперед, а осенью откатывайся назад”.
or
На рис. 4.10 показано, что происходит с часами. Когда мы переводим наши часы в марте, время движется вперед с 1:59 до 3:00. Когда мы переводим наши часы в ноябре, время возвращается назад, и мы переживаем час с 1:00 до 1:59 дважды. Дети усваивают это как «пружинить вперед весной и отступить осенью».

#### Рис. 4.10 - Как работает переход на летнее время

Например, 13 марта 2022 года мы переводим наши часы на час вперед и переходим с 2:00 утра на 3:00 утра. Это означает, 
что в этот день не будет 2:30 утра. Если бы мы хотели узнать время на час позже 1:30, то было бы 3:30.
...
Обратите внимание, что в этом примере меняются две вещи. Время меняется с 1:30 до 3:30. Смещение UTC также изменяется. 
Помните, мы рассчитывали время по Гринвичу, вычитая часовой пояс из времени? Вы можете видеть, что мы перешли от 6:30 по 
Гринвичу (1:30 минус -5:00) до 7:30 по Гринвичу (3:30 минус -4:00). Это показывает, что время действительно изменилось 
на один час с точки зрения GMT. Мы напечатали поля часов и смещений отдельно, чтобы подчеркнуть.

Аналогично, в ноябре час после первоначального 1:30 утра также равен 1:30 утра, потому что в 2:00 ночи мы повторяем этот
час. На этот раз попробуйте самостоятельно рассчитать время по Гринвичу для всех трех случаев, чтобы подтвердить, что 
мы действительно перемещаемся только на один час за раз.
...
Java достаточно умен, чтобы знать, что в эту ночь нет 2:30 утра, и переключается на соответствующее смещение по Гринвичу.

Да, раздражает, что Oracle ожидает, что вы будете знать это, даже если вы не находитесь в Соединенных Штатах — или, 
если уж на то пошло, в той части Соединенных Штатов, где не соблюдается летнее время. Создатели экзамена находятся в 
Соединенных Штатах, и они решили, что каждый должен знать, как работают часовые пояса США.

## Резюме

В этой главе вы узнали, что String — это неизменяемая последовательность символов. Явный вызов конструктора необязателен. 
Оператор конкатенации (+) создает новую строку с содержимым первой строки, за которой следует содержимое второй строки. 
Если любой операнд, участвующий в выражении +, является строкой, используется конкатенация; в противном случае 
используется дополнение. Строковые литералы хранятся в пуле строк. Класс String имеет множество методов.

Напротив, StringBuilder представляет собой изменяемую последовательность символов. Большинство методов возвращают 
ссылку на текущий объект, чтобы разрешить цепочку методов. У класса StringBuilder много методов.

Вызов == для объектов String проверит, указывают ли они на один и тот же объект в пуле.
Вызов == для ссылок StringBuilder проверит, указывают ли они на один и тот же объект StringBuilder.
Вызов equals() для объектов String проверит, является ли последовательность символов одинаковой.
Вызов equals() для объектов StringBuilder проверит, указывают ли они на один и тот же объект, а не просматривает 
значения внутри.

Массив — это область памяти фиксированного размера в куче, в которой есть место для примитивов или указателей на 
объекты. Вы указываете размер при его создании. Например, int[] a = new int[6];. Индексы начинаются с 0, а элементы 
обозначаются с помощью [0]. Массивы. Метод sort() сортирует массив. Arrays.binarySearch() выполняет поиск в 
отсортированном массиве и возвращает индекс совпадения. Если совпадений не найдено, то позиция, в которую должен быть 
вставлен элемент, отменяется, и вычитается 1. Arrays.compare() и Arrays.mismatch() проверяют эквивалентность двух 
массивов. Методы, которым передаются varargs (...), могут использоваться так, как если бы был передан обычный массив. 
В многомерном массиве массивы второго уровня и выше могут иметь разные размеры.

Класс Math предоставляет ряд статических методов для выполнения математических операций. Например, вы можете получить 
минимумы или максимумы. Вы можете округлять или даже генерировать случайные числа. Некоторые методы работают с любыми 
числовыми примитивами, а другие работают только с двойными.

LocalDate содержит только дату, LocalTime содержит только время, а LocalDateTime содержит и дату, и время. Все три имеют
частные конструкторы и создаются с помощью LocalDate.now() или LocalDate.of() (или эквивалентов для этого класса). 
Датой и временем можно управлять с помощью методов plusXXX или minusXXX. Класс Period представляет количество дней, 
месяцев или лет, которое нужно прибавить или вычесть из LocalDate или LocalDateTime. Все классы даты и времени являются 
неизменяемыми, что означает, что необходимо использовать возвращаемое значение.

## Основы экзамена

**Уметь определять вывод кода с помощью String.** Знайте правила конкатенации со String и как использовать общие методы 
String. Знайте, что строка неизменяема. Обратите особое внимание на тот факт, что индексы отсчитываются от нуля и что 
метод substring() получает строку прямо перед индексом второго параметра.

**Уметь определять вывод кода с помощью StringBuilder.** Знайте, что StringBuilder является изменяемым, и как 
использовать общие методы StringBuilder. Знайте, что substring() не изменяет значение StringBuilder, тогда как 
append(), delete() и insert() изменяют его. Также обратите внимание, что большинство методов StringBuilder возвращают 
ссылку на текущий экземпляр StringBuilder.

**Разберитесь в разнице между == и equals().** == проверяет равенство объектов. equals() зависит от реализации объекта, 
для которого он вызывается. Для класса String функция equals() проверяет символы внутри него.

**Уметь определять вывод кода с использованием массивов.** Знать, как объявлять и создавать экземпляры одномерных и 
многомерных массивов. Иметь доступ к каждому элементу и знать, когда индекс выходит за пределы. Распознавать правильные 
и неправильные выходные данные при поиске и сортировке.

**Определите возвращаемые типы методов Math.** В зависимости от переданного примитива методы Math могут возвращать 
разные примитивные результаты.

**Распознать недопустимое использование дат и времени.** LocalDate не содержит полей времени, а LocalTime не содержит 
полей даты. Следите за операциями, выполняемыми в неподходящее время. Также следите за добавлением или вычитанием 
времени и игнорированием результата. Освойте математику дат, включая часовые пояса и летнее время.
