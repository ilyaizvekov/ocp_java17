...
## Передача данных между методами (или Передача данных в методах)

Java — это язык с передачей по значению. Это означает, что создается копия переменной, и метод получает эту копию. 
Присваивания, сделанные в методе, не влияют на вызывающую сторону. Давайте посмотрим на пример:
...
В строке 3 num присваивается значение 4. В строке 4 мы вызываем метод. В строке 8 параметру num в методе присваивается 
значение 8. Хотя этот параметр имеет то же имя, что и переменная в строке 3, это совпадение. Имя может быть любым. 
На экзамене часто используется одно и то же имя, чтобы попытаться вас запутать. Переменная в строке 3 никогда не 
изменяется, поскольку ей не присваивается значение.

### Передача объектов

Теперь, когда вы познакомились с примитивами, давайте попробуем пример со ссылочным типом. Как вы думаете, что выводит 
следующий код?
...
Правильный ответ - Webby. Как и в примитивном примере, переменная присваивается только параметру метода и не влияет на 
вызывающую программу.

Обратите внимание, как мы продолжаем говорить о присвоении переменных. Это потому, что мы можем вызывать методы для 
параметров. В качестве примера, вот код, который вызывает метод StringBuilder, переданный в метод:
...
В этом случае speak() вызывает метод для параметра. Он не переназначает s другому объекту. На рис. 5.4 вы можете 
увидеть, как по-прежнему используется передача по значению. Переменная s является копией переменной name. Оба указывают 
на один и тот же StringBuilder, что означает, что изменения, внесенные в StringBuilder, доступны для обеих ссылок.

#### Рис. 5.4 - Копирование ссылки с передачей по значению


---

#### Передача по значению vs. передачи по ссылке

Разные языки обрабатывают параметры по-разному. Передача по значению используется во многих языках, включая Java. 
В этом примере метод swap() не изменяет исходные значения. Он изменяет только a и b внутри метода.
...
Другой подход — передача по ссылке. Он используется по умолчанию в нескольких языках, таких как Perl. Мы не собираемся 
показывать вам код Perl здесь, потому что вы готовитесь к экзамену по Java, и мы не хотим вас смущать. 
В языке передача по ссылке переменные поменяются местами, а вывод будет обратным.

---

Для обзора Java использует передачу по значению для получения данных в метод. Назначение нового примитива или ссылки на 
параметр не изменяет вызывающий объект. Вызов методов по ссылке на объект может повлиять на вызывающую сторону.

### Возврат объектов

Получить данные из метода проще. Создается копия примитива или ссылки и возвращается из метода. Это возвращаемое 
значение используется в большинстве случаев. Например, это может быть сохранено в переменной. Если возвращаемое значение
не используется, результат игнорируется. Обратите внимание на это на экзамене. Игнорируемые возвращаемые значения — это 
сложно.

Давайте рассмотрим пример. Обратите внимание на возвращаемые типы.
...
Это сложный вопрос, потому что нужно многое отслеживать. Когда вы увидите такие вопросы на экзамене, запишите значения 
каждой переменной. Строки 3 и 4 представляют собой простые задания. Строка 5 вызывает метод. Строка 10 увеличивает 
параметр метода до 3, но оставляет переменную ticket в методе main() равной 2. В то время как строка 11 возвращает 
значение, вызывающий объект игнорирует его. Вызов метода в строке 6 не игнорирует результат, поэтому guest становится 
"abcd". Помните, что это происходит из-за возвращаемого значения, а не из-за параметра метода.

### Автоупаковка и распаковка переменных (Autoboxing and Unboxing Variables)

Java поддерживает некоторые полезные функции, связанные с передачей примитивных типов данных и типов данных-оболочек, 
таких как int и Integer. Помните из главы 1, что мы можем явно преобразовывать примитивы в классы-оболочки, используя 
встроенные методы.
...
Полезно, но немного многословно. К счастью, в язык Java встроены обработчики, которые автоматически преобразуются между 
примитивами и классами-оболочками и обратно. Автобоксинг (Autoboxing) — это процесс преобразования примитива в 
эквивалентный класс-оболочку, а распаковка (unboxing) — процесс преобразования класса-оболочки в эквивалентный ему 
примитив.
...
Новый код эквивалентен предыдущему коду, поскольку компилятор «выполняет работу» по автоматическому преобразованию типов
за вас. Автоупаковка применяется ко всем примитивам и связанным с ними типам оболочек, таким как следующие:
...
Каждый из этих примеров компилируется без проблем. В последней строке e распаковывается до значения int. Поскольку 
значение int может быть сохранено в переменной long с помощью неявного приведения, компилятор разрешает присваивание.

---

#### Limits of Autoboxing and Numeric Promotion (Пределы автобоксинга и числового продвижения)

Хотя Java неявно преобразует меньший примитив в больший тип, а также автоматически упаковывает, он не будет делать и то,
и другое одновременно. Вы понимаете, почему следующее не компилируется?

```
Long badGorilla = 8; // DOES NOT COMPILE
```

Java автоматически преобразует или автоматически упакует значение int в long или Integer, соответственно. Однако ни один
из этих типов не может быть назначен ссылочной переменной Long, поэтому код не компилируется. Сравните это поведение с 
предыдущим примером с ears, где неупакованное примитивное значение может быть неявно приведено к более крупному 
примитивному типу.

---

Как вы думаете, что произойдет, если вы попытаетесь распаковать null?
...
В строке 10 мы сохраняем null в ссылке на Character. Это допустимо, поскольку null ссылка может быть присвоена любой 
ссылочной переменной. В строке 11 мы пытаемся распаковать этот null в примитив char. Это проблема. Java пытается 
получить значение char, равное null. Поскольку вызов любого метода с null значением приводит к исключению 
NullPointerException, именно это мы и получаем. Будьте осторожны, когда вы видите null в отношении автоупаковки и 
распаковки.

Где автобоксинг и распаковка действительно хороши, так это когда мы применяем их к вызовам методов.
...
В этом примере вызов climb() компилируется, потому что значение int может быть неявно приведено к типу long. 
Вызов swing() также разрешен, потому что значение int автоматически упаковывается в Integer. С другой стороны, вызов 
jump() приводит к ошибке компилятора, поскольку тип long должен быть явно приведен к типу int. Другими словами, Java не 
будет автоматически преобразовываться в более узкий тип.

Как и прежде, к вызовам методов применяется то же ограничение, связанное с автобоксингом и числовым расширением. 
...
Java автоматически выполняет приведение или автобокс значения, но не оба одновременно.

## Перегрузка методов

Теперь, когда вы знакомы с правилами объявления и использования методов, пришло время взглянуть на создание методов с 
одинаковыми именами в одном классе. Перегрузка методов происходит, когда методы в одном классе имеют одинаковое имя, но 
разные сигнатуры методов, что означает, что они используют разные списки параметров. (Перегрузка отличается от 
переопределения, о которой вы узнаете в главе 6.)

Мы уже давно показывали, как вызывать перегруженные методы. Методы System.out.println() и append() в StringBuilder 
предоставляют множество перегруженных версий, так что вы можете передать им практически что угодно, не задумываясь об 
этом. В обоих этих примерах единственным изменением был тип параметра. Перегрузка также допускает различное количество 
параметров.

Всё, кроме имени метода, может варьироваться для методов перегрузки. Это означает, что могут быть разные модификаторы 
доступа, необязательные спецификаторы (например, static), возвращаемые типы и списки исключений.

Ниже показаны пять перегруженных версий метода fly():
...
Как видите, мы можем перегрузить, изменив что-нибудь в списке параметров. У нас может быть другой тип, больше типов или 
те же типы в другом порядке. Также обратите внимание, что тип возвращаемого значения, модификатор доступа и список 
исключений не имеют отношения к перегрузке. Имеют значение только имя метода и список параметров.

Теперь давайте рассмотрим пример, который не является допустимой перегрузкой:
...
Этот метод не компилируется, потому что отличается от оригинала только типом возвращаемого значения. Сигнатуры методов 
одинаковы, поэтому они дублируют методы с точки зрения Java.

Как насчет этих; почему они не компилируются?
...
Опять же, сигнатуры этих трех методов одинаковы. Вы не можете объявлять методы в одном и том же классе, единственная 
разница в том, что один из них является методом экземпляра, а другой — static методом. Вы также не можете иметь два 
метода, которые имеют списки параметров с одинаковыми типами переменных и в одном и том же порядке. Как мы упоминали 
ранее, имена параметров в списке не имеют значения при определении сигнатуры метода.

Вызывать перегруженные методы несложно. Вы просто пишете код, и Java вызывает нужный. Например, посмотрите на эти два 
метода:
...
Вызов fly((short) 1) печатает short. Она ищет соответствующие типы и вызывает соответствующий метод. Конечно, это может 
быть сложнее, чем это.

Теперь, когда вы знаете основы перегрузки, давайте рассмотрим некоторые более сложные сценарии, с которыми вы можете 
столкнуться на экзамене.

### Ссылочные типы

Учитывая правило о том, что Java выбирает наиболее конкретную версию метода, какую только может, как вы думаете, что 
выводит этот код?
...
Ответ: строка-объект. Первый вызов передает строку и находит прямое совпадение. Нет причин использовать версию Object, 
когда есть хороший список параметров String, ожидающий вызова. Второй вызов ищет список параметров int. Когда он не 
находит его, он автоматически упаковывается в Integer. Поскольку он все ещё не находит соответствия, он переходит 
к Object.

Давай попробуем по-другому. Что здесь напечатано?
...
Ответ: CIO. Код предназначен для участия в акции! Первый вызов print() передает String. Как вы узнали из главы 4, String
и StringBuilder реализуют интерфейс CharSequence. Вы также узнали, что Arrays.asList() можно использовать для создания 
объекта List<Integer>, что объясняет второй вывод. Последний вызов print() передает LocalDate. Это класс, который вы 
можете не знать, но это нормально. Это явно не последовательность символов или список. Это означает, что используется 
сигнатура метода Object.

### Примитивы

Примитивы работают аналогично ссылочным переменным. Java пытается найти наиболее конкретный соответствующий 
перегруженный метод. Как вы думаете, что здесь происходит?
...
Ответ int-long. Первый вызов передает значение int и видит точное совпадение. Второй вызов передает long и также видит 
точное совпадение. Если мы закомментируем перегруженный метод списком параметров int, вывод станет long-long. В Java нет
проблем с вызовом более крупного примитива. Однако это не произойдет, если не будет найдено лучшее совпадение.

### Autoboxing (Автобоксинг)

Как мы видели ранее, автоупаковка применяется к вызовам методов, но что произойдет, если у вас есть как примитивная, так
...
Эти перегрузки метода допустимы. _Java пытается использовать наиболее конкретный список параметров, который может найти._
Это верно для автобоксинга, а также для других типов соответствия, о которых мы говорим в этом разделе.

Это означает, что вызов fly(3) вызовет первый метод. Когда примитивной версии int нет, Java будет автоматически 
упаковываться. Однако, когда предоставляется примитивная версия int, у Java нет причин выполнять дополнительную работу 
по автоупаковке.

### Массивы

В отличие от предыдущего примера, этот код не упаковывает автоматически:
...
Массивы существуют с самого начала Java. Они определяют их фактические типы. Как насчет универсальных типов, таких как 
List<Integer>? Мы рассмотрим эту тему в главе 9.

### Varargs

Как вы думаете, какой метод вызывается, если мы передаем int[]?
...
Вопрос с подвохом! Помните, что Java обрабатывает varargs как массив. Это означает, что сигнатура метода одинакова для 
обоих методов. Поскольку нам не разрешено перегружать методы с одним и тем же списком параметров, этот код не 
компилируется. Несмотря на то, что код выглядит по-разному, он компилируется в тот же список параметров.

Теперь, когда мы только что объяснили, что эти два метода похожи, пришло время упомянуть, в чем они различаются. 
Неудивительно, что вы можете вызвать любой метод, передав массив:

```
fly(new int[] { 1, 2, 3 });   // Allowed to call either fly() method
```

Однако вы можете вызвать версию varargs только с отдельными параметрами:

```
fly(1, 2, 3); // Allowed to call only the fly() method using varargs
```

Очевидно, это означает, что они не компилируются _точно_ так же. Однако список параметров тот же, и это то, что вам нужно 
знать о перегрузке для экзамена.

### Собираем все вместе

До сих пор все правила для вызова перегруженного метода должны быть логичными. Java вызывает наиболее конкретный метод, 
который может. Когда некоторые типы взаимодействуют друг с другом, правила Java сосредотачиваются на обратной 
совместимости. Давным-давно автобоксинга и varargs не существовало. Поскольку старый код все ещё должен работать, это 
означает, что автоупаковка и varargs появляются последними, когда Java просматривает перегруженные методы. Готовы к 
официальному заказу? Таблица 5.6 излагает это для вас.

#### Таблица 5.6 - Порядок, который Java использует для выбора правильного перегруженного метода

| Правило                   | Пример того, что будет выбрано для  glide(1,2) | 
|:--------------------------|:-----------------------------------------------|
| Точное совпадение по типу | String glide(int i, int j)                     |                                      
| Крупный примитивный тип   | String glide(long i, long j)                   |                                      
| Тип с автобоксом          | String glide(Integer i, Integer j)             |
| Varargs                   | String glide(int... nums)                      |

Давайте проверим это на практике, используя правила из таблицы 5.6. Как вы думаете, что это выводит?
...
Он выводит 142. Первый вызов соответствует сигнатуре, принимая одну строку, поскольку это наиболее точное совпадение. 
Второй вызов соответствует сигнатуре, принимающей два параметра String, поскольку это точное совпадение. 
Только в третьем вызове используется версия varargs, поскольку лучшего соответствия нет.

## Резюме 

В этой главе мы представили множество правил объявления методов и переменных. Методы начинаются с модификаторов доступа 
и необязательных спецификаторов в любом порядке (хотя обычно сначала модификаторы доступа). Модификаторы доступа, 
которые мы обсуждали в этой главе, бывают private, package (опущены), protected и public. Необязательный спецификатор 
для методов, которые мы рассмотрели в этой главе, является static. Мы рассмотрим дополнительные модификаторы методов в 
следующих главах.

Далее следует тип возвращаемого значения метода, который является void, если нет возвращаемого значения. Далее приведены
имя метода и список параметров, которые составляют уникальную сигнатуру метода.
Имя метода использует стандартные правила идентификатора Java, а список параметров состоит из нуля или более типов с 
именами. Необязательный список исключений также может быть добавлен после списка параметров. Наконец, блок определяет 
тело метода (опущенное для abstract методов).

Модификаторы доступа используются не только для методов, поэтому убедитесь, что вы хорошо их понимаете. Использование 
ключевого слова private означает, что код доступен только внутри того же класса. Доступ к пакету означает, что код 
доступен только из того же пакета. Использование ключевого слова protected означает, что код доступен из того же пакета 
или подклассов. Использование ключевого слова public означает, что код доступен из любого места.

И static методы, и static переменные являются общими для всех экземпляров класса. При обращении к ним извне класса они 
вызываются с использованием имени класса, например, Pigeon.fly(). Членам экземпляра разрешено вызывать static члены, но
static члены не могут вызывать члены экземпляра. Кроме того, static импорт используется для импорта static членов.

Мы также представили final модификатор и показали, как его можно применять к локальным переменным, переменным экземпляра
и static переменным. Помните, что локальная переменная является окончательной, если она не модифицируется после 
присвоения. Один быстрый тест для этого — добавить модификатор final и посмотреть, компилируется ли код.

Java использует передачу по значению, что означает, что вызовы методов создают копию параметров. Присвоение новых 
значений этим параметрам в методе не влияет на переменные вызывающей стороны. Вызов методов для объектов, являющихся 
параметрами метода, изменяет состояние этих объектов и отражается в вызывающем объекте. Java поддерживает автоупаковку и
распаковку примитивов и оболочек автоматически внутри метода и через вызовы методов.

Перегруженные методы - это методы с тем же именем, но другим списком параметров. Java вызывает наиболее конкретный 
метод, который может найти. Предпочтительны точные совпадения, за которыми следуют более широкие примитивы. После этого 
следует автобоксинг и, наконец, varargs.

Убедитесь, что вы поняли всё, что описано в этой главе. Это закладывает основу того, что вы узнаете в следующих главах.

## Основы экзамена

**Умейте определять правильные и неправильные объявления методов.** Уметь просмотреть сигнатуру метода и понимать, 
является ли она правильной, содержит ли она недопустимые или конфликтующие элементы, или содержит элементы в 
неправильном порядке.

**Идентифицируйте, когда метод или поле доступны.** Распознайте, когда метод или поле доступны, если модификатор 
доступа: private, package (опущен), protected или public.

**Поймите, как объявлять и использовать final переменные.** Локальные, переменные экземпляра и static переменные могут 
быть объявлены как final. Уметь понимать, как их объявлять и как их можно (или нельзя) использовать.

**Умейте эффективно определять final переменные.** Эффективно конечные переменные - это локальные переменные, которые не
изменяются после их назначения. Учитывая локальную переменную, будьте в состоянии определить, является ли она эффективно
окончательной.

**Распознавайте допустимое и недопустимое использование static импорта.** Статический импорт импортирует static 
элементы. Они записываются как import static, а не как static import. Убедитесь, что они импортируют static методы или 
переменные, а не имена классов.

**Применяйте автоматическую упаковку и распаковку.** Процесс автоматического преобразования примитивного значения в 
класс-оболочку называется автоупаковкой, а обратный процесс — распаковкой. Следите за NullPointerException при 
выполнении распаковки.

**Назовите вывод кода с использованием методов.** Определите, когда следует вызывать static методы, а не методы 
экземпляра, основываясь на том, идёт ли имя класса или объекта перед методом. Поймите, что методы экземпляра могут 
вызывать static методы и что static методам нужен экземпляр объекта для вызова метода экземпляра.

**Распознайте корректный перегруженный метод.** Сначала используются точные совпадения, затем следуют более широкие 
примитивы, затем автоупаковка, а затем varargs. Присвоение новых значений параметрам метода не изменяет вызывающую 
сторону, но вызывает для них методы.
