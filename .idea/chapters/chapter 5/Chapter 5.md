...
Первый способ хорош. По сути, это эквивалентно примеру с Лебедем. Гусь протягивает Птицу. Поскольку мы находимся в подклассе Goose и ссылаемся на ссылку Goose, он может получить доступ к защищенным членам. Второй способ - проблема. Хотя объект оказывается гусем, он хранится в ссылке на птицу. Нам не разрешено ссылаться на членов класса Bird, поскольку мы не находимся в одном пакете, а ссылочный тип other не является подклассом Goose.
...
Этот код не компилируется, потому что мы не находимся в объекте гуся. Метод floatInWater() объявлен в Bird. GooseWatcher не входит в тот же пакет, что и Bird, и не является его расширением. Гусь протягивает Птицу. Это позволяет Goose обращаться только к floatInWater(), а не к вызывающим объектам Goose.

Если это все еще озадачивает, попробуйте. Введите код и попробуйте его скомпилировать. Тогда перечитайте этот раздел. Не волнуйтесь — для нас это тоже было не очевидно в первый раз!
