...
Обратите внимание, что тип возвращаемого значения в дочернем методе отличается от String до int. В этом примере метод getSize() в родительском классе объявляется повторно, поэтому метод в дочернем классе является новым методом, а не переопределением метода в родительском классе.

Что, если бы метод getSize() был объявлен общедоступным в Beetle? В этом случае метод в RhinocerosBeetle будет недопустимым переопределением. Модификатор доступа в RhinocerosBeetle более строгий, а возвращаемые типы не являются ковариантными.
