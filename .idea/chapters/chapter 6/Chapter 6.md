...
### Повторное объявление private методов

Что произойдет, если вы попытаетесь переопределить private метод? В Java вы не можете переопределить private методы, 
поскольку они не наследуются. Тот факт, что дочерний класс не имеет доступа к родительскому методу, не означает, что 
дочерний класс не может определить свою собственную версию метода. Строго говоря, это просто означает, что новый метод 
не является переопределенной версией метода родительского класса.

Java позволяет повторно объявить новый метод в дочернем классе с той же или измененной сигнатурой, что и метод в 
родительском классе. Этот метод в дочернем классе является отдельным и независимым методом, не связанным с методом 
родительской версии, поэтому ни одно из правил переопределения методов не вызывается. Например, эти два объявления 
компилируются:
...
Обратите внимание, что тип возвращаемого значения в дочернем методе отличается от String до int. В этом примере метод 
getSize() в родительском классе объявляется повторно, поэтому метод в дочернем классе является новым методом, а не 
переопределением метода в родительском классе.

Что, если бы метод getSize() был объявлен public в Beetle? В этом случае метод в RhinocerosBeetle будет недопустимым 
переопределением. Модификатор доступа в RhinocerosBeetle более строгий, а возвращаемые типы не являются ковариантными.

### Скрытие static методов

Static метод нельзя переопределить, потому что объекты класса не наследуются друг от друга так же, как объекты 
экземпляра. С другой стороны, их можно скрыть. _Скрытый метод_ возникает, когда дочерний класс определяет static метод с
тем же именем и сигнатурой, что и унаследованный static метод, определенный в родительском классе. Сокрытие метода 
похоже на переопределение метода, но не совсем то же самое. Предыдущие четыре правила переопределения метода должны 
соблюдаться, когда метод скрыт. Кроме того, добавлено новое пятое правило для скрытия метода:

5. Метод, определенный в дочернем классе, должен быть помечен как static, если он помечен как static в родительском 
классе.

Проще говоря, это скрытие метода, если два метода помечены как static, и переопределение метода, если они не помечены 
как static. Если один помечен как static, а другой нет, класс не будет компилироваться.

Рассмотрим несколько примеров нового правила:
...
В этом примере код компилируется и запускается. Метод eat() в классе Panda скрывает метод eat() в классе Bear, печатая
"Panda is chewing" во время выполнения. Поскольку они оба помечены как static, это не считается переопределенным 
методом. Тем не менее, некоторое наследование все ещё происходит. Если вы удалите объявление eat() в классе Panda, то 
вместо этого программа напечатает "Bear is eating".

Посмотрите, сможете ли вы разобраться, почему каждое из объявлений метода в классе SunBear не компилируется:
...
В этом примере функция sneze() помечена как static в родительском классе, но не в дочернем классе. Компилятор 
обнаруживает, что вы пытаетесь переопределить метод экземпляра. Тем не менее, sneeze() — это static метод, который 
следует скрывать, иначе компилятор выдаст ошибку. Второй метод, hibernate(), не компилируется по другой причине. 
Метод помечен как static в дочернем классе, но не в родительском классе.

Наконец, метод laugh() не компилируется. Несмотря на то, что обе версии метода помечены как static, версия в SunBear 
имеет более ограничительный модификатор доступа, чем тот, который она наследует, и это нарушает второе правило для 
переопределения методов. Помните, что при сокрытии static методов необходимо соблюдать четыре правила переопределения 
методов.

### Сокрытие переменных

Как вы видели при переопределении методов, существует множество правил, когда два метода имеют одинаковую сигнатуру и 
определены как в родительском, так и в дочернем классе. К счастью, правила для переменных с одинаковыми именами в 
родительском и дочернем классах намного проще. На самом деле Java не позволяет переопределять переменные. Однако 
переменные можно скрыть.

_Скрытая переменная_ возникает, когда дочерний класс определяет переменную с тем же именем, что и унаследованная 
переменная, определенная в родительском классе. Это создает две разные копии переменной в экземпляре дочернего класса: 
один экземпляр, определенный в родительском классе, и один, определенный в дочернем классе.

Как и при сокрытии static метода, вы не можете переопределить переменную; можно только скрыть. Давайте посмотрим на 
сокрытую переменную. Как вы думаете, что печатает следующее приложение?
...
Запутались в выводе? Оба этих класса определяют переменную hasFur, но с разными значениями. Несмотря на то, что метод 
main() создает только один объект, обе переменные существуют независимо друг от друга. Выходные данные изменяются в 
зависимости от используемой ссылочной переменной.

Если вы не поняли последний пример, не волнуйтесь. Мы рассмотрим полиморфизм более подробно в следующей главе. На данный
момент вам просто нужно знать, что переопределение метода заменяет родительский метод для всех ссылочных переменных 
(кроме super), тогда как скрытие(или сокрытие) метода или переменной заменяет член, только если используется дочерний 
ссылочный тип.

### Написание final методов

Мы завершаем обсуждение наследования методов не требующим пояснений правилом: методы final не могут быть переопределены.
Помечая метод как final, вы запрещаете дочернему классу заменять этот метод. Это правило действует как при 
переопределении метода, так и при его скрытии. Другими словами, вы не можете скрыть static метод в дочернем классе, если
он помечен как final в родительском классе.
...
В этом примере метод экземпляра hasFeathers() помечен как final в родительском классе Bird, поэтому дочерний класс 
Penguin не может переопределить родительский метод, что приводит к ошибке компиляции. Static метод flyAway() также 
помечен как final, поэтому его нельзя скрыть в подклассе. В этом примере не имеет значения, использует ли дочерний метод
ключевое слово final — код в любом случае не будет компилироваться.

Это правило применяется только к унаследованным методам. Например, если два метода были помечены как private в 
родительском классе Bird, то класс Penguin, как он определен, будет компилироваться. В этом случае private методы будут
объявлены заново, а не переопределены или скрыты.

## Создание абстрактных классов

При разработке модели мы иногда хотим создать сущность, экземпляр которой нельзя создать напрямую. Например, 
представьте, что у нас есть класс Canine с подклассами Wolf, Fox и Coyote. Мы хотим, чтобы другие разработчики могли 
создавать экземпляры подклассов, но, возможно, мы не хотим, чтобы они могли создавать экземпляры Canine. Другими 
словами, мы хотим, чтобы все объекты Canine имели определенный тип во время выполнения.

### Введение в абстрактные классы

Введите абстрактные классы. Абстрактный класс — это класс, объявленный с модификатором abstract, который не может быть 
создан напрямую и может содержать абстрактные методы. Давайте рассмотрим пример, основанный на модели данных Canine:
...
В этом примере другие разработчики могут создавать экземпляры Wolf, Fox или Coyote, но не Canine. Конечно, они могут 
передавать ссылку на переменную как Canine, но базовый объект должен быть подклассом Canine во время выполнения.

Но подождите, это ещё не все! Абстрактный класс может содержать абстрактные методы. _Абстрактный метод_ — это метод, 
объявленный с модификатором abstract, который не определяет тело. Иными словами, абстрактный метод заставляет подклассы 
переопределять метод.

Зачем нам это? Полиморфизм, конечно! Объявляя абстрактный метод, мы можем гарантировать, что некоторая версия будет 
доступна в экземпляре без необходимости указывать, что это за версия в абстрактном родительском классе.
...
Затем мы можем создать экземпляр Fox и назначить его родительскому типу Canine. Переопределенный метод будет 
...
Пока что легко. Но есть некоторые правила, о которых нужно знать:

+ Только методы экземпляра могут быть помечены как abstract внутри класса, а не переменные, конструкторы или static методы.
+ Абстрактный метод может быть объявлен только в абстрактном классе.
+ Неабстрактный класс, расширяющий абстрактный класс, должен реализовывать все унаследованные абстрактные методы.
+ Переопределение абстрактного метода выполняется в соответствии с существующими правилами переопределения методов, о 
которых вы узнали ранее в этой главе.

Давайте посмотрим, сможете ли вы определить, почему каждое из этих объявлений класса не компилируется:
...
Во-первых, класс FennecFox не компилируется, потому что это недопустимое переопределение метода. В частности, 
возвращаемые типы не являются ковариантными. Класс ArcticFox не компилируется, поскольку он не переопределяет 
абстрактный метод getSound(). Класс Direwolf не компилируется, потому что он не абстрактный, а объявляет абстрактный 
метод rest(). Наконец, класс Jackal не компилируется, потому что переменные нельзя пометить как абстрактные.

Абстрактный класс чаще всего используется, когда вы хотите, чтобы другой класс наследовал свойства определенного класса,
но вы хотите, чтобы подкласс заполнил некоторые детали реализации.

Ранее мы говорили, что абстрактный класс — это класс, экземпляр которого не может быть создан. Это означает, что если 
вы попытаетесь создать его экземпляр, компилятор сообщит об исключении, как в этом примере:
...
Абстрактный класс может быть инициализирован, но только как часть создания экземпляра неабстрактного подкласса.

### Объявление абстрактных методов

Абстрактный метод всегда объявляется без тела. Он также включает точку с запятой (;) после объявления метода. Как вы 
видели в предыдущем примере, абстрактный класс может включать неабстрактные методы, в данном случае метод bark(). 
Фактически, абстрактный класс может включать в себя все те же члены, что и неабстрактный класс, включая переменные, 
static методы и методы экземпляра, конструкторы и т. д.

Вас может удивить тот факт, что абстрактный класс не обязан включать какие-либо абстрактные методы. Например, следующий 
код компилируется, даже если он не определяет никаких абстрактных методов:
...
Даже без абстрактных методов нельзя создать экземпляр класса напрямую. Для экзамена обратите внимание на абстрактные 
методы, объявленные вне абстрактных классов, например следующие (такими как следующие:):
...
Создатели экзамена любят включать недопустимые объявления классов, смешивая неабстрактные классы с абстрактными 
методами.

Как и модификатор final, модификатор abstract может быть помещен до или после модификатора доступа в объявлениях 
классов и методов, как показано в этом классе Tiger:
...
Модификатор abstract нельзя размещать после ключевого слова class в объявлении класса или после возвращаемого типа в 
объявлении метода. Следующие объявления Bear и Howl() не компилируются по этим причинам:
...
_Невозможно определить абстрактный метод, который имеет тело или реализацию по умолчанию. Вы все ещё можете определить 
метод по умолчанию с телом — вы просто не можете пометить его как abstract. Пока вы не пометите метод как final, 
подкласс имеет возможность переопределить унаследованный метод._

### Создание конкретного класса

Абстрактный класс становится пригодным для использования, когда он расширяется конкретным подклассом. _Конкретный класс_
— это неабстрактный класс. Первый конкретный подкласс, расширяющий абстрактный класс, необходим для реализации всех 
унаследованных абстрактных методов. Это включает в себя реализацию любых унаследованных абстрактных методов от 
унаследованных интерфейсов, как вы увидите в следующей главе.

Когда вы увидите на экзамене конкретный класс, расширяющий абстрактный класс, убедитесь, что он реализует все 
необходимые абстрактные методы. Вы понимаете, почему следующий класс Walrus не компилируется?
...
В этом примере мы видим, что Animal помечен как abstract, а Walrus — нет, что делает Walrus конкретным подклассом 
Animal. Поскольку Walrus — первый конкретный подкласс, он должен реализовать все унаследованные абстрактные методы — 
в данном примере getName(). Поскольку это не так, компилятор сообщает об ошибке при объявлении Walrus.

Мы выделяем первый конкретный подкласс не просто так. Абстрактный класс может расширять неабстрактный класс и наоборот. 
Каждый раз, когда конкретный класс расширяет абстрактный класс, он должен реализовать все методы, унаследованные как 
абстрактные. Давайте проиллюстрируем это на наборе унаследованных классов:
...
В этом примере класс BlackRhino является первым конкретным подклассом, а классы Mammal и Rhino являются абстрактными. 
Класс BlackRhino наследует метод eatLeaf() как абстрактный, поэтому от него требуется реализация, что он и делает.

Как насчет метода showHorn()? Поскольку родительский класс Rhino предоставляет реализацию showHorn(), этот метод 
наследуется в BlackRhino как неабстрактный метод. По этой причине классу BlackRhino разрешается, но не требуется 
переопределять метод showHorn(). Три класса в этом примере правильно определены и компилируются.

Что, если мы изменим объявление Rhino, убрав модификатор abstract?
...
Преобразовав Rhino в конкретный класс, он становится первым неабстрактным классом, расширяющим абстрактный класс Mammal. 
Следовательно, он должен обеспечивать реализацию обоих методов showHorn() и eatLeaf(). Поскольку он предоставляет 
только один из этих методов, измененное объявление Rhino не компилируется.

Давайте попробуем ещё один пример. Следующий конкретный класс Lion наследует два абстрактных метода, getName() и roar():
...
В этом примере кода BigCat расширяет Animal, но помечен как abstract; поэтому не требуется предоставлять реализацию для 
метода getName(). Класс Lion не помечен как abstract, и как первый конкретный подкласс он должен реализовывать все 
унаследованные абстрактные методы, не определенные в родительском классе. Все три из этих классов успешно компилируются.

### Создание конструкторов в абстрактных классах

Несмотря на то, что абстрактные классы не могут быть созданы, они все равно инициализируются через конструкторы их 
подклассами. Например, рассмотрим следующую программу:
...
Используя правила конструктора, о которых вы узнали ранее в этой главе, компилятор вставляет конструктор по умолчанию 
без аргументов в класс Platypus, который сначала вызывает super() в классе Mammal. Конструктор Mammal вызывается только 
тогда, когда абстрактный класс инициализируется через подкласс; таким образом, во время вызова конструктора существует 
реализация chew(). Этот код компилируется и печатает yummy! во время выполнения.

Для экзамена помните, что абстрактные классы инициализируются с помощью конструкторов так же, как и неабстрактные 
классы. Например, если абстрактный класс не предоставляет конструктор, компилятор автоматически вставит конструктор по 
умолчанию без аргументов.

Основное различие между конструктором в абстрактном классе и неабстрактном классе заключается в том, что конструктор в 
абстрактном классе можно вызывать только тогда, когда он инициализируется неабстрактным подклассом. Это имеет смысл, 
поскольку абстрактные классы не могут быть созданы.

### Выявление недействительных объявлений

Мы завершаем обсуждение абстрактных классов обзором потенциальных проблем, с которыми вы, скорее всего, столкнетесь на 
экзамене, чем в реальной жизни. Составителям экзаменов нравятся вопросы с методами, помеченными как abstract, для 
которых также определена реализация. Например, видите ли вы, почему каждый из следующих методов не компилируется?
...
Первый метод, eat(), не компилируется, потому что он помечен как abstract, но не заканчивается точкой с запятой (;). 
Следующие два метода, swim() и getAge(), не компилируются, поскольку они помечены как abstract, но они предоставляют 
блок реализации, заключенный в фигурные скобки ({}). Для экзамена помните, что объявление абстрактного метода должно 
заканчиваться точкой с запятой без фигурных скобок. Следующий метод, sleep, не компилируется из-за отсутствия круглых 
скобок () для аргументов метода. Последний метод, goInShell(), не компилируется, поскольку он не помечен как abstract 
и, следовательно, должен предоставить тело, заключенное в фигурные скобки.

Убедитесь, что вы понимаете, почему каждый из предыдущих методов не компилируется, и что вы можете обнаружить подобные 
ошибки на экзамене. Если вы столкнетесь с вопросом на экзамене, в котором класс или метод помечен как abstract, 
убедитесь, что класс правильно реализован, прежде чем пытаться решить проблему.

#### Модификаторы abstract и final

Что произойдет, если вы пометите класс или метод как abstract, так и final? Если вы отмечаете что-то abstract, вы 
предполагаете, что кто-то другой расширит или реализует это. Но если вы отмечаете что-то final, вы мешаете кому-либо 
расширять или реализовывать это. Эти понятия находятся в прямом противоречии друг с другом.

Из-за этой несовместимости Java не позволяет помечать класс или метод как abstract, так и final. Например, следующий 
фрагмент кода не скомпилируется:
...
В этом примере ни объявление класса, ни объявления метода не будут компилироваться, потому что они помечены как 
abstract, так и final. На экзамене редко используются модификаторы final в классах или методах, поэтому, если вы их 
видите, убедитесь, что они не используются с модификатором abstract.

#### Модификаторы abstract и private

Метод не может быть помечен одновременно как abstract и private. Это правило имеет смысл, если подумать. 
Как бы вы определили подкласс, который реализует требуемый метод, если этот метод не наследуется подклассом? Ответ 
заключается в том, что вы не можете, поэтому компилятор будет жаловаться, если вы попытаетесь сделать следующее:
...
В этом примере абстрактный метод sing(), определенный в родительском классе Whale, невидим для подкласса HumpbackWhale. 
Несмотря на то, что HumpbackWhale предоставляет реализацию, она не считается переопределением абстрактного метода, 
поскольку абстрактный метод не наследуется. Компилятор распознает это в родительском классе и сообщает об ошибке, как 
только private и abstract применяются к одному и тому же методу.

_Хотя невозможно объявить метод abstract и private, возможно (хотя и избыточно) объявить метод final и private._

Если мы изменим модификатор доступа с private на protected в родительском классе Whale, скомпилируется ли код?
...
В этом модифицированном примере код всё равно не скомпилируется, но совсем по другой причине. Если вы помните правила 
переопределения метода, подкласс не может уменьшить видимость родительского метода, sing(). Поскольку метод объявлен
protected в родительском классе, он должен быть помечен как protected или public в дочернем классе. Даже с абстрактными 
методами необходимо соблюдать правила переопределения методов.

#### Модификаторы abstract и static

Как мы обсуждали ранее в этой главе, static метод можно только скрыть, но не переопределить. Он определяется как 
принадлежащий классу, а не экземпляр класса. Если static метод нельзя переопределить, то из этого следует, что его 
также нельзя пометить как abstract, поскольку он никогда не может быть реализован. Например, следующий класс не 
компилируется:
...
Перед экзаменом убедитесь, что вы знаете, какие модификаторы можно и нельзя использовать друг с другом, особенно для 
абстрактных классов и интерфейсов.

## Создание неизменяемых объектов

Как вы, возможно, помните из главы 4, неизменяемый объект — это объект, состояние которого не может измениться после 
его создания. _Шаблон неизменяемых объектов_ — это шаблон объектно-ориентированного проектирования, в котором объект не 
может быть изменен после его создания.

Неизменяемые объекты полезны при написании безопасного кода, поскольку вам не нужно беспокоиться об изменении значений. 
Они также упрощают код при работе с параллелизмом, поскольку неизменяемые объекты могут быть легко разделены между 
несколькими потоками.

### Объявление неизменяемого класса

Хотя существует множество способов написания неизменяемого класса, вы должны быть знакомы с общей стратегией создания 
неизменяемого класса:

1. Отметьте класс как final или сделайте все конструкторы private.
2. Отметьте все переменные экземпляра как private и final.
3. Не определяйте никаких методов-сеттеров.
4. Не позволяйте изменять ссылочные изменяемые объекты.
5. Используйте конструктор, чтобы установить все свойства объекта, при необходимости создав копию.

Первое правило не позволяет никому создавать изменяемый подкласс. Второе и третье правила гарантируют, что вызывающие 
объекты не вносят изменений в переменные экземпляра, и являются отличительными чертами хорошей инкапсуляции — темы, 
которую мы обсудим вместе с записями в главе 7.

Четвертое правило создания неизменяемых объектов тонкое. По сути, это означает, что вы не должны предоставлять метод 
доступа (или метод получения) для изменяемых полей экземпляра. Вы понимаете, почему следующее создает изменяемый объект?
...
Мы тщательно следовали первым трем правилам, но, к сожалению, злоумышленник все еще может изменить наши данные:
...
О, нет! Зебрам нельзя есть Chocolate Chip Cookies! Это не неизменяемый объект, если мы можем изменить его содержимое! 
Если у нас нет геттера для объекта FavoriteFoods, как вызывающие могут получить к нему доступ? Просто: используя методы 
делегата или оболочки для чтения данных.
...
В этой улучшенной версии данные по-прежнему доступны. Однако это настоящий неизменяемый объект, поскольку вызывающая 
сторона не может изменить изменяемую переменную.

---

#### Копирование при чтении методов доступа

Помимо делегирования доступа к любым private изменяемым объектам, другой подход заключается в создании копии изменяемого
объекта каждый раз, когда он запрашивается.
...
Конечно, изменения в копии не отразятся на оригинале, но, по крайней мере, оригинал защищен от внешних изменений. 
Это может быть дорогостоящей операцией, если вызывающая сторона часто вызывает её.

---

### Выполнение защитного копирования

Итак, что насчет пятого и последнего правила создания неизменяемых объектов? Допустим, при разработке нашего класса нам 
нужно правило, согласно которому данные для FavoriteFoods предоставляются вызывающей стороной и что они всегда содержат 
хотя бы один элемент. Это правило часто называют инвариантом; это верно каждый раз, когда у нас есть экземпляр объекта.
...
Чтобы убедиться, что favoriteFoods предоставлен, мы проверяем его в конструкторе и выбрасываем исключение, если он не 
предоставлен. Так это неизменно? Не совсем! Злоумышленник может быть хитрым и хранить свою секретную ссылку на наш 
объект favoriteFoods, который он может изменить напрямую.
...
Упс! Похоже, что Animal больше не является неизменным, поскольку его содержимое может измениться после его создания. 
Решение состоит в том, чтобы сделать копию объекта списка, содержащего те же элементы.
...
Операция копирования называется _защитной копией_, потому что копия создается на случай, если другой код сделает что-то 
неожиданное. Это та же идея, что и при защитном вождении: предотвратить проблему до того, как она возникнет.
При таком подходе наш класс Animal снова становится неизменяемым.

## Резюме

В этой главе мы взяли основные структуры классов, представленные в книге, и расширили их, введя понятие наследования. 
Классы Java следуют шаблону единого наследования, в котором каждый класс имеет ровно один прямой родительский класс, 
при этом все классы в конечном итоге наследуются от java.lang.Object.

Наследование класса дает вам доступ ко всем public и protected членам класса. Это также дает вам доступ к членам пакета 
класса, если классы находятся в одном пакете. Все методы экземпляра, конструкторы и инициализаторы экземпляра имеют 
доступ к двум специальным ссылочным переменным: this и super. И this, и super предоставляют доступ ко всем 
унаследованным членам, и только this обеспечивает доступ ко всем членам в текущем объявлении класса.

Конструкторы — это специальные методы, которые используют имя класса и не имеют возвращаемого типа. Они используются для
создания экземпляров новых объектов. Объявление конструкторов требует соблюдения ряда важных правил. Если конструктор не
указан, компилятор автоматически вставит в класс конструктор по умолчанию без аргументов. Первая строка каждого 
конструктора — это вызов перегруженного конструктора this() или родительского конструктора super(); в противном случае 
компилятор вставит вызов super() в качестве первой строки конструктора. В некоторых случаях, например, если родительский
класс не определяет конструктор без аргументов, это может привести к ошибкам компиляции. На экзамене обратите особое 
внимание на любой класс, который определяет конструктор с аргументами и не определяет конструктор без аргументов.

Классы инициализируются в заранее определенном порядке: инициализация суперкласса; static переменные и static 
инициализаторы в том порядке, в котором они появляются; переменные экземпляра и инициализаторы экземпляра в том порядке,
в котором они появляются; и, наконец, конструктор. Всем final переменным экземпляра должно быть присвоено значение 
ровно один раз.

Мы рассмотрели перегруженные, переопределенные, скрытые и переобъявленные методы и показали, чем они отличаются. 
Метод считается перегруженным, если он имеет то же имя, но другую сигнатуру, что и другой доступный метод. Метод 
переопределяется, если он имеет ту же сигнатуру, что и унаследованный метод, с совместимыми модификаторами доступа, 
исключениями и возвращаемым типом. Static метод скрыт, если он имеет ту же сигнатуру, что и унаследованный static метод.
Наконец, метод объявляется повторно, если он имеет то же имя и, возможно, ту же сигнатуру, что и ненаследуемый метод.

Затем мы перешли к абстрактным классам, которые похожи на обычные классы, за исключением того, что они не могут быть 
созданы и могут содержать абстрактные методы. Абстрактный класс может расширять неабстрактный класс и наоборот. 
Абстрактные классы можно использовать для определения структуры, для которой другие разработчики пишут подклассы. 
Абстрактный метод — это метод, который не включает тело при объявлении. Абстрактный метод может быть размещен только 
внутри абстрактного класса или интерфейса. Затем абстрактный метод может быть переопределен другим абстрактным 
объявлением или конкретной реализацией при условии соблюдения правил переопределения методов. Первый конкретный класс 
должен реализовать все унаследованные абстрактные методы, независимо от того, унаследованы ли они от абстрактного класса
или интерфейса.

Наконец, в этой главе показано, как создавать неизменяемые объекты в Java. Хотя для этого существует несколько различных
методов, мы включили наиболее распространенный, который вам следует знать перед экзаменом. Неизменяемые объекты 
чрезвычайно полезны на практике, особенно в многопоточных приложениях, поскольку они не изменяются.

## Основы экзамена

**Умейте писать код, расширяющий другие классы.** Класс Java, который расширяет другой класс, наследует все его 
public и protected методы и переменные. Если класс находится в том же пакете, он также наследует все члены пакета этого 
класса. Классы, помеченные как final, не могут быть расширены. Наконец, все классы в Java расширяют java.lang.Object 
либо напрямую, либо из суперкласса.

**Уметь различать и использовать this, this(), super, и super().** Для доступа к текущему или унаследованному члену 
класса можно использовать ссылку this. Для доступа к унаследованному элементу можно использовать суперссылку. 
Суперссылка часто используется для уменьшения неоднозначности, например, когда класс повторно использует имя 
унаследованного метода или переменной. Вызовы this() и super() используются для доступа к конструкторам в том же классе 
и родительском классе соответственно.

**Оценивайте код, в котором используются конструкторы.** Первая строка каждого конструктора — это вызов другого 
конструктора внутри класса с помощью this() или вызов конструктора родительского класса с помощью вызова super(). 
Компилятор вставит вызов super(), если вызов конструктора не объявлен. Если родительский класс не содержит конструктора 
без аргументов, необходимо обеспечить явный вызов родительского конструктора. Уметь распознавать, когда предоставляется 
конструктор по умолчанию. Помните, что порядок инициализации заключается в инициализации всех классов в иерархии 
классов, начиная с суперкласса. Затем экземпляры инициализируются, опять же начиная с суперкласса. Всем final переменным
должно быть присвоено значение ровно один раз к моменту завершения конструктора.

**Изучите правила переопределения методов.** Java позволяет переопределять методы или заменять их подклассом, если 
соблюдаются определенные правила: метод должен иметь ту же сигнатуру, быть по крайней мере таким же доступным, что и 
родительский метод, не должен объявлять никаких новых или более широких исключений и должен использовать ковариантные 
возвращаемые типы. Методы, помеченные как final, не могут быть переопределены или скрыты.

**Знайте разницу между переопределением метода и перегрузкой метода.** Как перегрузка, так и переопределение метода 
включают создание нового метода с тем же именем, что и у существующего метода. Когда сигнатура метода одинакова, это 
называется переопределением метода и для компиляции должно следовать определенному набору правил переопределения. 
Когда сигнатура метода отличается, и метод принимает разные входные данные, это называется перегрузкой метода, и ни одно
из правил переопределения не требуется. Переопределение метода важно для полиморфизма, потому что оно заменяет все 
вызовы метода, даже сделанные в суперклассе.

**Изучите правила сокрытия методов и переменных.** Когда static метод переопределяется в подклассе, это называется 
сокрытием метода. Точно так же сокрытие переменной происходит, когда унаследованное имя переменной повторно используется
в подклассе. В обоих случаях исходный метод или переменная по-прежнему существуют и доступны в зависимости от того, где 
к ним осуществляется доступ, и от используемого ссылочного типа. Для сокрытия метода использование static в объявлении 
метода должно быть одинаковым для родительского и дочернего классов. Наконец, следует избегать сокрытия переменных и 
методов, так как это приводит к запутанному и сложному коду.

**Умейте писать код, который создает и расширяет абстрактные классы.** В Java классы и методы могут быть объявлены как 
abstract. Абстрактный класс не может быть создан. Экземпляр абстрактного класса можно получить только через конкретный 
подкласс. Абстрактные классы могут включать любое количество абстрактных и неабстрактных методов, включая ноль. 
Абстрактные методы следуют всем правилам переопределения методов и могут быть определены только внутри абстрактных 
классов. Первый конкретный подкласс абстрактного класса должен реализовывать все унаследованные методы. Абстрактные 
классы и методы не могут быть помечены как final.

**Создавайте неизменяемые объекты.** Неизменяемый объект — это объект, который нельзя изменить после его объявления. 
Неизменяемый класс обычно реализуется с помощью private конструктора, без методов установки и без возможности изменять 
изменяемые объекты, содержащиеся в классе.
