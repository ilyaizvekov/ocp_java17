...
#### Таблица 8.8 - Правила доступа к переменной из лямбда-тела внутри метода
...
В этой главе мы потратили много времени на то, чтобы научить вас использовать лямбда-выражения, и не зря. Следующие две 
главы во многом зависят от вашей способности создавать и использовать лямбда-выражения. Мы рекомендуем вам хорошо понять 
эту главу, прежде чем двигаться дальше.

Лямбда-выражения, или лямбды, позволяют передавать блоки кода. Полный синтаксис выглядит так:

```
(String a, String b) -> { return a.equals(b); }
```

Типы параметров можно не указывать. Если указан только один параметр без типа, круглые скобки также можно опустить. 
Фигурные скобки и оператор return могут быть опущены для одного оператора, что делает краткую форму следующим образом:

```
a -> a.equals(b)
```

Лямбды можно передать методу, ожидающему экземпляр функционального интерфейса. Лямбда может определять параметры или 
переменные в теле, если их имена отличаются от существующих локальных переменных. В теле лямбды разрешено использовать 
любые переменные экземпляра или класса.

Кроме того, он может использовать любые локальные переменные или параметры метода, которые являются final или 
effectively final.

Ссылка на метод — это компактный синтаксис для написания лямбда-выражений, ссылающихся на методы. Существует четыре т
ипа: static методы, методы экземпляра для определенного объекта, методы экземпляра для параметра и ссылки на 
конструктор.

Функциональный интерфейс имеет один абстрактный метод. Любой функциональный интерфейс можно реализовать с помощью 
лямбда-выражения. Вы должны знать встроенные функциональные интерфейсы.

Вам следует просмотреть таблицы в главе. Хотя существует множество таблиц, некоторые из них имеют общие шаблоны, что 
упрощает их запоминание. Вы обязательно должны запомнить таблицу 8.4, в которой перечислены общие функциональные 
интерфейсы.

## Основы экзамена

**Пишите простые лямбда-выражения.** Ищите наличие или отсутствие необязательных элементов в лямбда-коде. Типы 
параметров являются необязательными. Фигурные скобки и ключевое слово return необязательны, если тело представляет 
собой один оператор. Скобки необязательны, если указан только один параметр и тип неявный.

**Определите, можно ли использовать переменную в теле лямбда.** Локальные переменные и параметры метода должны быть 
final или effectively final, чтобы на них можно было ссылаться. Это означает, что код должен скомпилироваться, если вы 
добавите ключевое слово final к этим переменным. Переменные экземпляра и класса разрешены всегда.

**Преобразование ссылок на методы в лямбда-выражения «длинной формы».** Уметь преобразовывать ссылки на методы в 
обычные лямбда-выражения и наоборот. Например, System.out::print и x -> System.out.print(x) эквивалентны. Помните, что 
порядок параметров метода выводится при использовании ссылки на метод.

**Определите, является ли интерфейс функциональным интерфейсом.** Используйте правило единого абстрактного метода (SAM), 
чтобы определить, является ли интерфейс функциональным интерфейсом. Другие типы методов интерфейса (default, private, 
static и private static) не учитываются при подсчете одного абстрактного метода, как и любые public методы с 
сигнатурами, найденными в Object.

**Определите правильный функциональный интерфейс, учитывая количество параметров, тип возвращаемого значения и имя 
метода, и наоборот.** Наиболее распространенными функциональными интерфейсами являются Supplier, Consumer, Function и 
Predicate. Существуют также двоичные версии и примитивные версии многих из этих методов. Вы можете использовать 
количество параметров и тип возвращаемого значения, чтобы отличить их друг от друга.
