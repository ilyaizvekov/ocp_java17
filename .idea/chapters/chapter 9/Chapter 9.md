...
### Передача обобщенных аргументов

Теперь о методах. Тот же вопрос: попытайтесь выяснить, почему они не компилируются и что они делают. Мы представим 
методы по одному, потому что есть над чем задуматься.
...
Первый метод, first(), представляет собой совершенно нормальное использование дженериков. Он использует параметр типа T, 
специфичный для метода. Он принимает параметр List<T> или некоторый подкласс T и возвращает один объект этого типа T. 
Например, вы можете вызвать его с параметром List<String> и вернуть String. Или вы можете вызвать его с параметром 
List<Number> и вернуть Number. Или… ну, вы поняли.

Учитывая это, вы сможете увидеть, что не так с этим:

```
<T> <? extends T> second(List<? extends T> list) { // НЕ КОМПИЛИРУЕТСЯ   
    return list.get(0);
}
```

Следующий метод, second(), не компилируется, поскольку возвращаемый тип на самом деле не является типом. Вы пишете 
метод. Вы знаете, какой тип он должен возвращать. Вы не можете указать это как подстановочный знак.

Теперь будьте осторожны - это очень сложно:

```
<B extends A> B third(List<B> list) {    
    return new B(); // НЕ КОМПИЛИРУЕТСЯ
}
```

Этот метод third() не компилируется. <B extends A> говорит, что вы хотите использовать B в качестве параметра типа 
только для этого метода и что ему необходимо расширить класс A. По совпадению, B также является названием класса. 
Что ж, это не совпадение. Это злая уловка. В рамках метода B может представлять класс A, B или C, поскольку все они 
расширяют класс A. Поскольку B больше не ссылается на класс B в методе, вы не можете создать его экземпляр.

После этого было бы неплохо получить что-то простое.

```
void fourth(List<? super B> list) {}
```

Наконец мы получаем метод four(), который представляет собой обычное использование дженериков. Вы можете передать тип
List<B>, List<A>, или List<Object>.

Наконец, можете ли вы понять, почему этот пример не компилируется?

```
<X> void fifth(List<X super B> list) { // НЕ КОМПИЛИРУЕТСЯ 
}
```

Этот последний метод, fifth(), не компилируется, поскольку он пытается смешать параметр типа, специфичный для метода, с 
подстановочным знаком. Подстановочный знак должен иметь ? в этом.

Уф. Вы сделали это с помощью дженериков. Это самая сложная тема в этой главе (и почему мы рассмотрели ее в последнюю 
очередь!). Помните, что ничего страшного, если вам придется просмотреть этот материал несколько раз, чтобы усвоить его.

## Резюме

Java Collections Framework включает четыре основных типа структур данных: списки, наборы, очереди и карты. Интерфейс 
Collection является родительским интерфейсом List, Set и Queue. Кроме того, Deque расширяет Queue. Интерфейс Map 
не расширяет Collection. Вам необходимо осознать следующее:

+ List: Упорядоченный набор элементов, допускающий дублирование записей
+ ArrayList: Стандартный список изменяемого размера
+ LinkedList: Можно легко добавить/удалить с начала или конца

+ Set: Не допускает дубликатов
+ HashSet: Использует hashCode() для поиска неупорядоченных элементов
+ TreeSet: Сортировка. Не разрешает нулевые значения

+ Queue/Deque: Элементы заказов для обработки
+ ArrayDeque: Двусторонняя очередь
+ LinkedList: Двусторонняя очередь и список

+ Map: Сопоставляет уникальные ключи со значениями
+ HashMap: Использует hashCode() для поиска ключей
+ TreeMap: Сортировка карты. Не позволяет использовать нулевые ключи

Интерфейс Comparable объявляет метод compareTo(). Этот метод возвращает отрицательное число, если объект меньше своего 
аргумента, 0, если два объекта равны, и положительное число в противном случае. Метод compareTo() объявлен для 
сравниваемого объекта и принимает один параметр. Интерфейс Comparator определяет метод compare(). Отрицательное число 
возвращается, если первый аргумент меньше, ноль, если они равны, и положительное число в противном случае. Метод 
compare() может быть объявлен в любом коде и принимает два параметра. Comparator часто реализуется с использованием 
лямбды.

Обобщения — это параметры типа для кода. Чтобы создать класс с универсальным параметром, добавьте <T> после имени 
класса. Вы можете использовать любое имя для параметра типа. Одиночные заглавные буквы являются распространенным 
выбором. Обобщенные шаблоны позволяют указывать подстановочные знаки. <?> — это неограниченный подстановочный знак, 
означающий любой тип. <? extends Object> — это верхняя граница, которая означает любой тип, который является Object или 
расширяет его. <? extends MyInterface> означает любой тип, реализующий MyInterface. <? super Number> — это нижняя 
граница, которая означает любой тип Number или суперкласс. Ошибка компилятора возникает в результате кода, который 
пытается добавить элемент в список с неограниченным или ограниченным сверху подстановочным знаком.

## Основы экзамена

**Выберите правильную коллекцию типов из описания.** List позволяет дублировать и упорядочивать элементы. Set не 
допускает дубликатов. Deque упорядочивает свои элементы для облегчения поиска спереди или сзади. Map сопоставляет 
ключи со значениями. Ознакомьтесь с различиями в реализациях этих интерфейсов.

**Используйте удобные методы.** Платформа Collections Framework содержит множество методов, таких как contains(), 
forEach() и removeIf(), которые вам необходимо знать для экзамена. Их слишком много, чтобы их можно было 
перечислить в этом параграфе, поэтому просмотрите таблицы в этой главе.

**Различайте Comparable и Comparator.** Говорят, что классы, реализующие Comparable, имеют естественный порядок и 
реализуют метод compareTo(). Классу разрешено иметь только один естественный порядок. Comparator принимает два объекта в 
методе compare(). Разные могут иметь разные порядки сортировки. Comparator часто реализуется с использованием 
лямбды, например (a, b) -> a.num - b.num.

**Определите допустимое и недопустимое использование обобщенных шаблонов и подстановочных знаков.** <T> представляет 
параметр типа. Можно использовать любое имя, но принято использовать одну заглавную букву. <?> — неограниченный 
подстановочный знак. <? extends X> — это подстановочный знак, ограниченный сверху. <? super X> — это подстановочный знак
с нижней границей.
